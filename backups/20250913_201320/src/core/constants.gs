 ...
 * ⚠️ 注意: PROPS_KEYSは /src/constants.gs で既に定義されているため、
 * 重複回避のためここでは定義を削除しました。
 * 使用時は /src/constants.gs のPROPS_KEYSを参照してください。
 */
 ...
 *
 * ⚠️ 注意: CONSTANTSは /src/constants.gs で既に定義されているため、
 * 重複回避のためCONSTANTS_COREという名前で定義します
 */
// eslint-disable-next-line no-unused-vars
const CONSTANTS_CORE = Object.freeze({
...
// ✅ 簡素化：直接CONSTANTS_COREを使用（エイリアス削除でメモリ効率化）
// 使用例: CONSTANTS_CORE.REACTIONS.KEYS, CONSTANTS_CORE.COLUMNS, CONSTANTS_CORE.DATABASE.DELETE_LOG
//
// ⚠️ 注意: 実際のプロジェクトでは /src/constants.gs のCONSTANTSを使用してください
//         このファイルのCONSTANTS_COREは将来的な拡張機能用です
