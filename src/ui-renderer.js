...
const UI_CONSTANTS = {
  ...
  // GAS環境向け最適化
  GAS_RENDER_BATCH_SIZE: 8, // GAS環境では控えめに
...
class UIRenderer {
  ...
  async renderBoard(data, options = {}) {
    ...
    try {
      ...
      console.log(`✅ Board render completed in ${renderTime.toFixed(2)}ms`);

    } catch (error) {
      console.error('❌ Render error:', error);
      this.coreState.setError(error, 'UI Rendering');
      this.showErrorMessage('表示の更新中にエラーが発生しました');
    } finally {
  ...
  calculateVisibleRange(totalItems) {
    ...
    const itemHeight = 120; // 推定カード高さ
    const visibleItems = Math.ceil(containerHeight / itemHeight);
    const buffer = Math.ceil(visibleItems * 0.5); // 50%のバッファ
