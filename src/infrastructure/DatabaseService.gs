...
const DB_CONFIG = Object.freeze({
  CACHE_TTL: 30000, // 30秒 - CORE.TIMEOUTS.LONG 相当
  BATCH_SIZE: 100,
...
function getSheetsServiceCached() {
  ...
  try {
    ...
    console.warn('キャッシュアクセスエラー:', error.message);
  }
  ...
  try {
    ...
    try {
      ...
      console.error('Service Accountトークン取得エラー:', tokenError.message);
      throw new Error('Service Account認証が利用できません。システム管理者に連絡してください。');
    }

    // Service Objectトークン取得
      ...
      spreadsheets: {
        ...
        values: {
          ...
          append(params) {
            ...
            const url = `https://sheets.googleapis.com/v4/spreadsheets/${params.spreadsheetId}/values/${params.range}:append`;
    ...
    // キャッシュに保存（短期TTL）
    AppCacheService.set(cacheKey, serviceObject, AppCacheService.TTL.SHORT);

    return serviceObject;
  } catch (error) {
    console.error('getSheetsServiceCached: サービス生成エラー:', error.message);
    throw error;
  }
}
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error.message.includes('Service object corruption')) {
        ...
        CacheService.getScriptCache().remove('sheets_service');
      }
    }
...
const DB = {
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (row[userIdIndex] === userId) {
          ...
          // キャッシュに保存
          try {
            AppCacheService.set(cacheKey, JSON.stringify(userObj), DB_CONFIG.CACHE_TTL / 1000);
          } catch (cacheError) {
      ...
      // 見つからなかった場合
      try {
        AppCacheService.set(cacheKey, 'null', 60);
      } catch (cacheError) {
  ...
  findUserByEmail(email, forceRefresh = false) {
    ...
    if (!forceRefresh) {
      try {
        const cachedUser = AppCacheService.get(cacheKey);
        if (cachedUser !== null) {
    ...
    const logPrefix = forceRefresh
      ? '🔄 findUserByEmail: 強制更新'
    ...
    try {
      ...
      for (const row of userRows) {
        if (row[emailIndex] === email) {
          ...
          try {
            AppCacheService.set(cacheKey, user, 300);
          } catch (cacheError) {
...
function initializeDatabaseSheet(spreadsheetId) {
  try {
    ...
    try {
      ...
      // ヘッダーがない場合は設定
      if (
        !batchGetResult.valueRanges[0].values ||
        batchGetResult.valueRanges[0].values.length === 0
      ) {
...
function debugShowAllUsers() {
  try {
    ...
    try {
      ...
      if (
        deletionLogResult.valueRanges &&
        deletionLogResult.valueRanges[0] &&
        deletionLogResult.valueRanges[0].values
      ) {
      ...
      if (responseCode === 403) {
        try {
          ...
          if (
            ...
            // ServiceAccountのメール取得を試行
            let serviceAccountEmail = 'サービスアカウント';
            try {
              const props = PropertiesService.getScriptProperties();
              const saCredsString = props.getProperty(PROPS_KEYS.SERVICE_ACCOUNT_CREDS);
              if (saCredsString) {
                const saCreds = JSON.parse(saCredsString);
                serviceAccountEmail = saCreds.client_email || 'サービスアカウント';
              }
            } catch (emailError) {
              console.warn('ServiceAccountメール取得エラー:', emailError.message);
            }
