...
const PERFORMANCE_CONFIG = {
  MAX_PARALLEL_REQUESTS: 28,     // 並列リクエスト最大数（Apps Script API制限）
  ASYNC_TIMEOUT_MS: 300000,      // 5分（非同期処理タイムアウト）
  TRIGGER_DELAY_MS: 1000,        // 1秒（トリガー遅延）
 ...
 */
function executeAsync(functionName, params = {}, delayMs = PERFORMANCE_CONFIG.TRIGGER_DELAY_MS) {
  try {
    // パラメータをPropertiesServiceに一時保存
    const taskId = Utilities.getUuid();
    const paramsKey = `async_task_${taskId}`;
    
    PropertiesService.getScriptProperties().setProperty(paramsKey, JSON.stringify({
    ...
    // 時間ベーストリガーを作成
    ScriptApp.newTrigger('executeAsyncTask')
      .timeBased()
      .after(delayMs)
      .create();
    
    debugLog(`Async task scheduled: ${functionName} (${taskId})`);
    return taskId;
    
  } catch (e) {
    debugLog(`Async execution setup failed: ${e.message}`);
    throw e;
  }
}
...
function executeAsyncTask() {
  try {
    ...
    // async_taskで始まるプロパティを検索
    for (const key in allProps) {
      if (key.startsWith('async_task_')) {
        const taskData = JSON.parse(allProps[key]);
        const age = Date.now() - taskData.timestamp;
        
        // 5分以内のタスクのみ実行
        if (age < PERFORMANCE_CONFIG.ASYNC_TIMEOUT_MS) {
          try {
            // 関数を動的実行
            const func = this[taskData.functionName];
            if (typeof func === 'function') {
              debugLog(`Executing async task: ${taskData.functionName}`);
              func.apply(this, [taskData.params]);
            }
          } catch (execError) {
            debugLog(`Async task execution error: ${execError.message}`);
          }
        }
    ...
    triggers.forEach(trigger => {
      if (trigger.getHandlerFunction() === 'executeAsyncTask') {
        ScriptApp.deleteTrigger(trigger);
      }
    });
    
  } catch (e) {
    debugLog(`Async task execution failed: ${e.message}`);
  }
}

/**
 * 並列処理実行（fetchAll利用）
 * @param {Array} tasks - タスク配列 [{url, payload}, ...]
 * @param {number} maxConcurrent - 最大並列数
 */
function executeParallel(tasks, maxConcurrent = PERFORMANCE_CONFIG.MAX_PARALLEL_REQUESTS) {
  if (!tasks || tasks.length === 0) return [];
  
  const startTime = Date.now();
  debugLog(`Executing ${tasks.length} tasks in parallel (max: ${maxConcurrent})`);
  ...
  try {
    ...
    batches.forEach((batch, batchIndex) => {
      ...
      debugLog(`Executing batch ${batchIndex + 1}/${batches.length} (${requests.length} requests)`);
      
      const batchStart = Date.now();
      const responses = UrlFetchApp.fetchAll(requests);
      const batchTime = Date.now() - batchStart;
      
      debugLog(`Batch ${batchIndex + 1} completed in ${batchTime}ms`);
      ...
      const batchResults = responses.map((response, index) => {
        try {
          if (response.getResponseCode() === 200) {
            ...
            debugLog(`Request ${index} failed: ${response.getResponseCode()}`);
            return { error: `HTTP ${response.getResponseCode()}` };
          }
        } catch (parseError) {
          debugLog(`Response parse error: ${parseError.message}`);
    ...
    debugLog(`Parallel execution completed: ${tasks.length} tasks in ${totalTime}ms`);
    
    return allResults;
    
  } catch (e) {
    debugLog(`Parallel execution error: ${e.message}`);
    throw e;
  }
}
 ...
 */
function processBatch(data, processor, batchSize = PERFORMANCE_CONFIG.BATCH_SIZE) {
  if (!data || data.length === 0) return [];
  
  const startTime = Date.now();
  debugLog(`Processing ${data.length} items in batches of ${batchSize}`);
  ...
  try {
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      // バッチ内での並列処理（JavaScript配列操作）
      const batchResults = batch.map(processor);
      results.push(...batchResults);
      
      const batchTime = Date.now() - batchStart;
      debugLog(`Batch ${Math.floor(i / batchSize) + 1} processed in ${batchTime}ms`);
    }
    
    const totalTime = Date.now() - startTime;
    debugLog(`Batch processing completed: ${data.length} items in ${totalTime}ms`);
    
    return results;
    
  } catch (e) {
    debugLog(`Batch processing error: ${e.message}`);
    throw e;
  }
}
...
function registerNewUser(adminEmail) {
  const startTime = Date.now();
  debugLog('registerNewUser: Starting execution');
  
  checkRateLimit('registerNewUser', adminEmail);
  
  // 📝 ステップ1: API経由でのデータベースアクセス（キャッシュ活用）
  ...
  try {
    ...
    const existingCheckCacheKey = `existing_user_${adminEmail}`;
    const existingCheck = getCachedData(
      existingCheckCacheKey,
      () => callDatabaseApi('checkExistingUser', { adminEmail: adminEmail }),
      60 // 1分間キャッシュ（新規登録中の重複チェック用）
    );
  ...
  // 📝 ステップ3: Googleフォームとスプレッドシートを作成（最適化版）
  debugLog(`📝 フォーム・スプレッドシート作成開始: ${adminEmail}`);
  const formCreationStart = Date.now();
  
  const formAndSsInfo = createStudyQuestFormOptimized(adminEmail, userId);
  
  const formCreationTime = Date.now() - formCreationStart;
  debugLog(`✅ フォーム・スプレッドシート作成完了 (${formCreationTime}ms)`);
  ...
  debugLog(`🎉 新規登録完了: ${adminEmail} (ユーザーID: ${userId}) - 総実行時間: ${totalExecutionTime}ms`);
...
function createStudyQuestFormOptimized(userEmail, userId) {
  const startTime = Date.now();
  debugLog('createStudyQuestFormOptimized: Starting execution');
  ...
  try {
    ...
    // ユーザー専用フォルダを取得または作成（キャッシュ活用）
    const folderCacheKey = `user_folder_${userEmail}`;
    const userFolder = getCachedData(
      folderCacheKey,
      () => getUserFolder(userEmail),
      CACHE_CONFIG.USER_INFO_TTL
    );
    
    debugLog(`User folder obtained: ${userFolder.getId()}`);
    ...
    // フォームとスプレッドシートを並行作成（可能な部分）
    debugLog('Creating form and spreadsheet...');
    
    // Step 1: フォーム作成
    const formCreationStart = Date.now();
    const form = FormApp.create(`StudyQuest - みんなの回答ボード - ${userPrefix} - ${dateTimeString}`);
    const formId = form.getId();
    debugLog(`Form created: ${formId} (${Date.now() - formCreationStart}ms)`);
    
    // Step 2: フォーム設定を一括適用（効率化）
    const formSetupStart = Date.now();
    setupFormConfigurationBatch(form);
    debugLog(`Form configuration completed (${Date.now() - formSetupStart}ms)`);
    
    // Step 3: スプレッドシート作成
    const spreadsheetCreationStart = Date.now();
    const spreadsheet = SpreadsheetApp.create(`StudyQuest - みんなの回答ボード - 回答データ - ${userPrefix} - ${dateTimeString}`);
    const spreadsheetId = spreadsheet.getId();
    debugLog(`Spreadsheet created: ${spreadsheetId} (${Date.now() - spreadsheetCreationStart}ms)`);
    
    // Step 4: フォームとスプレッドシートの関連付け
    const linkingStart = Date.now();
    form.setDestination(FormApp.DestinationType.SPREADSHEET, spreadsheetId);
    debugLog(`Form-Spreadsheet linking completed (${Date.now() - linkingStart}ms)`);
    
    // Step 5: ファイル移動を並行実行（DriveApp最適化）
    ...
    try {
      formFile.moveTo(userFolder);
      spreadsheetFile.moveTo(userFolder);
      debugLog(`Files moved to user folder (${Date.now() - moveStart}ms)`);
    } catch (moveError) {
      debugLog('File move warning (continuing):', moveError.message);
    }
    
    // Step 6: URLs生成
    const urlsStart = Date.now();
    const formUrl = form.getPublishedUrl();
    const editFormUrl = 'https://docs.google.com/forms/d/' + formId + '/edit';
    const spreadsheetUrl = spreadsheet.getUrl();
    debugLog(`URLs generated (${Date.now() - urlsStart}ms)`);
    
    const totalTime = Date.now() - startTime;
    debugLog(`createStudyQuestFormOptimized completed in ${totalTime}ms`);
    ...
    debugLog('createStudyQuestFormOptimized error:', error.message);
    // フォールバック：元の関数を使用
    debugLog('Falling back to original createStudyQuestForm...');
 ...
 * フォーム設定をバッチで高速適用
 */
function setupFormConfigurationBatch(form) {
  try {
    // 基本設定を一括適用
    form.setCollectEmail(true);
    form.setRequireLogin(true);
    ...
    try {
      if (typeof form.setEmailCollectionType === 'function') {
        form.setEmailCollectionType(FormApp.EmailCollectionType.VERIFIED);
        debugLog('✅ setEmailCollectionType(VERIFIED) applied');
      }
    } catch (undocumentedError) {
      debugLog('⚠️ Undocumented method error:', undocumentedError.message);
    }
...
`.trim();
    ...
    // フォーム項目を一括追加（効率化）
    addFormItemsBatch(form);
    
    debugLog('Form configuration batch completed');
    
  } catch (e) {
    debugLog('Form configuration error:', e.message);
    throw e;
  }
}
 ...
 * フォーム項目をバッチで追加
 */
function addFormItemsBatch(form) {
  try {
    // クラス入力項目
    const classItem = form.addTextItem();
    classItem.setTitle('クラス名');
    ...
    debugLog('Form items added successfully');
    
  } catch (e) {
    debugLog('Form items addition error:', e.message);
    throw e;
  }
}
 ...
 * 超高速化版新規ユーザー登録関数
 * - 並列処理とトリガーベース非同期実行
 * - UrlFetchApp.fetchAllによる並列API呼び出し
 * - 高度なキャッシュ戦略
 */
function registerNewUserUltraFast(adminEmail) {
  const startTime = Date.now();
  debugLog('registerNewUserUltraFast: Starting ultra-fast execution');
  
  checkRateLimit('registerNewUser', adminEmail);
  
  try {
    // ステップ1: 既存ユーザーチェック（超高速キャッシュ）
    const existingCheckCacheKey = `existing_user_ultra_${adminEmail}`;
    const existingCheck = getCachedData(
      existingCheckCacheKey,
      () => callDatabaseApi('checkExistingUser', { adminEmail: adminEmail }),
      30 // 30秒の短期キャッシュ（登録処理中の重複防止）
    );
    ...
    // ステップ2: 新規ユーザー作成（並列処理版）
    const userId = Utilities.getUuid();
    if (!userId || typeof userId !== 'string' || userId.trim() === '') {
      throw new Error('ユーザーID生成に失敗しました。再度お試しください。');
    }
    
    debugLog(`Ultra-fast: Generated userId: ${userId}`);
    
    // ステップ3: 並列タスクの準備
    const parallelTasks = [
      // タスク1: フォーム・スプレッドシート作成（非同期で実行）
      {
        name: 'createFormAndSpreadsheet',
        execute: () => createStudyQuestFormUltraFast(adminEmail, userId)
      },
      // タスク2: ユーザーフォルダ準備（キャッシュ活用）
      {
        name: 'prepareUserFolder',
        execute: () => getCachedData(
          `user_folder_ultra_${adminEmail}`,
          () => getUserFolder(adminEmail),
          CACHE_CONFIG.USER_INFO_TTL
        )
      }
    ];
    
    // ステップ4: JavaScript並列処理による高速実行
    debugLog('Ultra-fast: Executing parallel tasks...');
    ...
    parallelTasks.forEach(task => {
      try {
        ...
        debugLog(`Task ${task.name} failed: ${taskError.message}`);
        results[task.name] = { error: taskError.message };
      }
    });
    
    const parallelTime = Date.now() - parallelStart;
    debugLog(`Ultra-fast: Parallel tasks completed in ${parallelTime}ms`);
    
    const formAndSsInfo = results.createFormAndSpreadsheet;
    if (formAndSsInfo.error) {
      throw new Error(`フォーム作成エラー: ${formAndSsInfo.error}`);
    }
    
    // ステップ5: データベース登録（非同期実行）
    const userData = {
      userId: userId,
      adminEmail: adminEmail,
      spreadsheetId: formAndSsInfo.spreadsheetId,
    ...
    // 非同期でデータベース登録を実行（ユーザーを待たせない）
    executeAsync('createUserViaApiAndSetup', {
      userData: userData,
      userId: userId,
      spreadsheetUrl: formAndSsInfo.spreadsheetUrl
    }, 500); // 0.5秒後に実行
    
    // ステップ6: ユーザーコンテキスト設定（即座に実行）
    PropertiesService.getUserProperties().setProperty('CURRENT_USER_ID', userId);
    PropertiesService.getUserProperties().setProperty('CURRENT_SPREADSHEET_ID', formAndSsInfo.spreadsheetId);
    ...
    const totalTime = Date.now() - startTime;
    debugLog(`registerNewUserUltraFast completed in ${totalTime}ms (background tasks continuing)`);
    ...
    return {
      ...
      adminUrl: adminUrl,
      viewUrl: viewUrl,
      newSheetName: 'フォームの回答 1', // デフォルトシート名
      autoCreated: true,
      message: '新しいボードが作成されました！設定は背景で続行されます。管理画面に移動します。'
    };
    
  } catch (error) {
    const totalTime = Date.now() - startTime;
    debugLog(`registerNewUserUltraFast failed in ${totalTime}ms: ${error.message}`);
    
    // エラー時は従来の方法にフォールバック
    debugLog('Falling back to standard registerNewUser...');
    return registerNewUser(adminEmail);
  }
}

/**
 * 非同期バックグラウンドタスク：ユーザー作成とセットアップ
 */
function createUserViaApiAndSetup(params) {
  try {
    debugLog('Background task: Creating user and setup...');
    
    const { userData, userId, spreadsheetUrl } = params;
    ...
    // 監査ログ
    auditLog('NEW_USER_REGISTERED', userId, { 
      adminEmail: userData.adminEmail, 
      spreadsheetId: userData.spreadsheetId 
    });
    
    // スプレッドシートリストに追加
    const addResult = addSpreadsheetUrl(spreadsheetUrl);
    ...
    debugLog('Background task completed successfully');
    
  } catch (error) {
    debugLog(`Background task failed: ${error.message}`);
  }
}
 ...
 * 超高速フォーム作成（最小限の処理）
 */
function createStudyQuestFormUltraFast(userEmail, userId) {
  const startTime = Date.now();
  debugLog('createStudyQuestFormUltraFast: Starting ultra-fast form creation');
  
  try {
    // 最小限の設定でフォーム作成
    const now = new Date();
    const dateTimeString = Utilities.formatDate(now, Session.getScriptTimeZone(), 'MM/dd HH:mm');
    const userPrefix = userEmail.split('@')[0];
    
    // フォーム作成（最小構成）
    const form = FormApp.create(`StudyQuest - ${userPrefix} - ${dateTimeString}`);
    const formId = form.getId();
    ...
    // 基本項目のみ追加（高速版）
    form.addTextItem().setTitle('クラス名').setRequired(true);
    form.addTextItem().setTitle('名前').setRequired(true);
    form.addParagraphTextItem().setTitle('回答').setRequired(true);
    
    // スプレッドシート作成
    const spreadsheet = SpreadsheetApp.create(`StudyQuest回答-${userPrefix}-${dateTimeString}`);
    const spreadsheetId = spreadsheet.getId();
    ...
    const totalTime = Date.now() - startTime;
    debugLog(`createStudyQuestFormUltraFast completed in ${totalTime}ms`);
    ...
    debugLog(`createStudyQuestFormUltraFast error: ${error.message}`);
    throw error;
  }
}
