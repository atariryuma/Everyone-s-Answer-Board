...
class ManagedExecutionContext {
  ...
  _trackMemoryUsage(resourceKey) {
    try {
      ...
      if (this.memoryUsage.current > 5000) {
        ULog.warn('⚠️ ExecutionContext高メモリ使用量検出:', this.memoryUsage.current);
      }
    } catch (error) {
      ULog.warn('メモリ使用量追跡エラー:', error.message);
    }
  }
  ...
  _scheduleAutoCleanup() {
    this.cleanupTimer = setTimeout(() => {
      if (!this.isDestroyed) {
        ULog.warn('⚠️ ExecutionContext自動クリーンアップ実行:', this.requestUserId);
        this.destroy();
      }
    }, this.maxLifetime);
  }
  ...
  removeResource(key) {
    if (this.resources.has(key)) {
      try {
        ...
        ULog.debug(`🗑️ リソース削除: ${key}`);
      } catch (error) {
        ULog.warn('リソース削除エラー:', key, error.message);
      }
    }
  }
    ...
    try {
      ...
      for (const [key, resource] of this.resources) {
        try {
          ...
          ULog.warn(`リソースクリーンアップエラー (${key}):`, cleanupError.message);
        }
      }
      ...
      ULog.debug(
        `✅ ExecutionContextクリーンアップ完了: ${this.requestUserId} (lifetime: ${lifetime}ms, peak memory: ${this.memoryUsage.peak})`
      );
    } catch (error) {
      logError(
        error,
        'ExecutionContextCleanup',
  ...
  isHealthy() {
    ...
    return (
      ...
      this.memoryUsage.current < 10000
    ); // メモリ制限チェック
  }
...
const globalContextManager = {
  ...
  register(context) {
    if (this.activeContexts.size >= this.maxConcurrentContexts) {
      ULog.warn('⚠️ 最大コンテキスト数到達、古いコンテキストを強制クリーンアップ');
      this._cleanupOldest();
    }

    this.activeContexts.set(context.requestUserId, context);
    ULog.debug(`📝 コンテキスト登録: ${context.requestUserId} (total: ${this.activeContexts.size})`);
  },
  ...
  unregister(userId) {
    if (this.activeContexts.has(userId)) {
      ...
      ULog.debug(`🗑️ コンテキスト削除: ${userId} (remaining: ${this.activeContexts.size})`);
    }
  },
...
function getCurrentSpreadsheet(requestUserId) {
  ...
  try {
    ...
    ULog.debug('🔧 getCurrentSpreadsheet: スプレッドシートアクセス試行中:', userInfo.spreadsheetId);

    try {
      return openSpreadsheetOptimized(userInfo.spreadsheetId);
    } catch (openError) {
      console.error('[ERROR]', '❌ SpreadsheetApp.openById 権限エラー:', openError.message);

      // 詳細なエラー情報とデバッグ情報を提供
      ULog.debug('🔍 権限エラー診断情報:', {
      ...
      if (
        openError.message.includes('openById') ||
        openError.message.includes('permission') ||
        openError.message.includes('Unexpected error')
      ) {
...
詳細エラー: ${openError.message}`);
    ...
    console.error('[ERROR]', 'getCurrentSpreadsheet エラー: ' + e.message);
...
function openActiveSpreadsheet(requestUserId) {
  ...
  try {
    ...
    console.error('[ERROR]', 'openActiveSpreadsheet エラー: ' + e.message);
  ...
  try {
    ...
    try {
      ...
      for (const [key, value] of Object.entries(allProperties)) {
        // 古い形式のユーザーキャッシュキーを削除
        if (
          key.startsWith('CURRENT_USER_ID_') &&
          key !==
            `CURRENT_USER_ID_${Utilities.computeDigest(
          ...
          ULog.debug('古い孤立ユーザーキーを削除: ' + key.substring(0, 20) + '...');
        }
      }
    } catch (propertyError) {
      ULog.warn('プロパティクリーンアップエラー:', propertyError.message);
    }
    ...
    if (scriptCache) {
      try {
        ...
        ULog.debug('スクリプトキャッシュクリーンアップスキップ:', scriptCacheError.message);
      }
    }

    ULog.debug('古いユーザーキャッシュをクリアしました: ' + currentEmail);
  } catch (e) {
    ULog.warn('キャッシュクリア中にエラーが発生しましたが、処理を続行します: ' + e.message);
  }
}
...
function getEffectiveSpreadsheetId(requestUserId) {
  verifyUserAccess(requestUserId);
  const userInfo = findUserById(requestUserId);
  const configJson = userInfo && userInfo.configJson ? JSON.parse(userInfo.configJson) : {};
...
function getSheetHeaders(requestUserId, spreadsheetId, sheetName) {
  ...
  if (lastColumn < 1) {
    ULog.warn(`シート '${sheetName}' に列が存在しません`);
    return [];
  }
...
function getConfig(requestUserId, sheetName, forceRefresh = false) {
  ...
  if (!forceRefresh) {
    ...
    if (cached) {
      ULog.debug('設定キャッシュヒット: %s', cacheKey);
      return JSON.parse(cached);
    }
  }

  try {
    ULog.debug('設定を取得中: sheetName=%s, userId=%s', sheetName, userId);
    const userInfo = findUserById(userId); // 依存関係を明確化
    ...
    if (savedSheetConfig && Object.keys(savedSheetConfig).length > 0) {
      // 3.【最優先】保存済みの設定を適用する
      ULog.debug('保存済み設定を適用します:', JSON.stringify(savedSheetConfig));
      ...
      ULog.debug('保存済み設定がないため、自動マッピングを実行します。');
    ...
    ULog.debug('getConfig: 最終設定を返します: %s', JSON.stringify(finalConfig));
    return finalConfig;
  } catch (error) {
    console.error('[ERROR]', 'getConfigでエラー:', error.message, error.stack);
...
function autoMapHeaders(headers, sheetName = null) {
  ...
  if (!result.opinionHeader) {
    const candidateHeaders = processedHeaders.filter(
      (h) => !usedHeaders.has(h.index) && !h.isMetadata && h.cleaned.length > 0
    );
  ...
  if (sheetName) {
    try {
      ...
      ULog.warn('データ内容分析でエラーが発生しましたが、処理を続行します:', e.message);
    }
  }
...
function isMetadataColumn(header) {
  ...
  return metadataPatterns.some((pattern) => headerLower.includes(pattern.toLowerCase()));
}
...
function calculateFallbackPriority(headerInfo) {
  ...
  // 質問らしいパターン
  if (
    headerInfo.lower.includes('ください') ||
    headerInfo.lower.includes('何') ||
    headerInfo.lower.includes('どう') ||
    headerInfo.lower.includes('?')
  ) {
    priority += 15;
  }
    ...
    processedHeaders.forEach((headerInfo, index) => {
      ...
      try {
        const columnData = sheet
          .getRange(2, index + 1, lastRow - 1, 1)
          .getValues()
        ...
        analysis[index] = {
          ...
          containsReasonWords: columnData.some(
            (val) =>
              val.includes('なぜなら') ||
              val.includes('理由') ||
              val.includes('から') ||
          ...
          containsClassPattern: columnData.some(
            (val) => /^\d+[年組]/.test(val) || /^[A-Z]\d*$/.test(val)
          ),
          dataCount: columnData.length,
        };
      } catch (e) {
        ULog.warn(`列${index + 1}の分析でエラー:`, e.message);
      }
    });

    return analysis;
  } catch (e) {
    ULog.warn('データ内容分析でエラー:', e.message);
    return {};
  }
}
...
function refineResultsWithContentAnalysis(result, contentAnalysis, processedHeaders) {
  ...
  if (refinedResult.opinionHeader) {
    const opinionIndex = processedHeaders.findIndex(
      (h) => h.original === refinedResult.opinionHeader
    );
    const analysis = contentAnalysis[opinionIndex];

    if (analysis && analysis.avgLength < 10 && !analysis.hasLongText) {
      // 短いテキストばかりの場合、本当に回答列か確認
      const betterCandidate = findBetterOpinionColumn(
        contentAnalysis,
        processedHeaders,
        refinedResult
      );
  ...
  if (refinedResult.reasonHeader) {
    const reasonIndex = processedHeaders.findIndex(
      (h) => h.original === refinedResult.reasonHeader
    );
    const analysis = contentAnalysis[reasonIndex];

    if (analysis && analysis.containsReasonWords) {
      // 理由を示すキーワードが含まれている場合、信頼度を上げる
      ULog.debug('理由列の信頼度が高いことを確認しました');
    }
  }
  ...
  if (refinedResult.classHeader) {
    ...
    if (analysis && analysis.containsClassPattern) {
      ULog.debug('クラス列でクラス番号パターンを確認しました');
    }
  }
 ...
 * @returns {object} { success: true|'error', message: string, [data]: object } 形式のオブジェクト
 */
function saveAndActivateSheet(requestUserId, spreadsheetId, sheetName, config) {
  ...
  try {
    ULog.debug('saveAndActivateSheet開始: sheetName=%s', sheetName);
    ...
    ULog.debug('saveAndActivateSheet: バッチ処理完了');

    // 5. 最新のステータスをキャッシュを無視して取得
    const finalStatus = getAppConfig(requestUserId);
    ULog.debug('saveAndActivateSheet: 統合処理完了');
    ...
    if (finalStatus && isNewOrUpdatedForm) {
      ULog.debug('saveAndActivateSheet: 新規/更新フォーム作成後の自動化開始');
    ...
    console.error('[ERROR]', 'saveAndActivateSheetで致命的なエラー:', error.message, error.stack);
...
function republishBoard(requestUserId) {
  ...
  return executeWithStandardizedLock('BATCH_OPERATION', 'republishBoard', () => {
    ULog.debug('republishBoard開始: userId=%s', requestUserId);
    ...
    ULog.debug('republishBoard完了: シート=%s', configJson.publishedSheetName);
...
function checkIfNewOrUpdatedForm(requestUserId, spreadsheetId, sheetName) {
  ...
  try {
    ...
    if (currentSpreadsheetId !== spreadsheetId) {
      ULog.debug('checkIfNewOrUpdatedForm: 新しいスプレッドシート検出');
      return true;
    }
    ...
    if (currentSheetName !== sheetName) {
      ULog.debug('checkIfNewOrUpdatedForm: 新しいシート検出');
      return true;
    }
    ...
    if (timeDiff < thirtyMinutes) {
      ULog.debug('checkIfNewOrUpdatedForm: 最近作成されたフォーム検出');
      return true;
    }

    ULog.debug('checkIfNewOrUpdatedForm: 既存フォームの設定更新');
    return false;
  } catch (error) {
    console.error('[ERROR]', 'checkIfNewOrUpdatedForm error:', error.message);
    // エラーの場合は安全側に倒して新規扱い
    return true;
  }
}
...
function saveDraftConfig(requestUserId, sheetName, config) {
  ...
  try {
    ...
    ULog.debug('saveDraftConfig開始: sheetName=%s', sheetName);
    ...
    ULog.debug('saveDraftConfig: 設定保存完了');

    return createSuccessResponse(null, '設定が下書きとして保存されました。');
  } catch (error) {
    console.error('[ERROR]', 'saveDraftConfigで致命的なエラー:', error.message, error.stack);
...
function activateSheet(requestUserId, spreadsheetId, sheetName) {
  ...
  try {
    ...
    ULog.debug('activateSheet開始: sheetName=%s', sheetName);
    ...
    if (currentUserId) {
      ...
      if (userInfo) {
        ...
        ULog.debug('activateSheet: 最小限キャッシュクリア完了');
      }
    }

    // シートをアクティブ化（効率化）
    const switchResult = switchToSheet(requestUserId, spreadsheetId, sheetName);
    ULog.debug('activateSheet: シート切り替え完了');
    ...
    try {
      ...
      if (userInfoForForm) {
        ...
        if (!cfg[sheetKey].formUrl) {
          ...
          if (detected && detected.success && detected.formUrl) {
            ...
            ULog.info(
              '✅ activateSheet: フォームURL自動設定 (sheet=' + sheetName + '): ' + detected.formUrl
            );
          } else {
            ULog.debug('ℹ️ activateSheet: フォームURL検出なし (sheet=' + sheetName + ')');
          }
        }
      ...
      ULog.warn('activateSheet: フォームURL同期で警告:', formSyncError.message);
    }

    // 最新のステータスを取得（キャッシュ活用）
    const finalStatus = getAppConfig(requestUserId);
    ULog.debug('activateSheet: 公開処理完了');

    const executionTime = Date.now() - startTime;
    ULog.debug('activateSheet完了: 実行時間 %dms', executionTime);
    ...
    console.error(
      '[ERROR]',
      'activateSheetで致命的なエラー (実行時間: %dms):',
      executionTime,
      error.message,
      error.stack
    );
...
function autoMapSheetHeaders(requestUserId, sheetName, overrides) {
  ...
  try {
    const headers = getSheetHeaders(
      requestUserId,
      getCurrentSpreadsheet(requestUserId).getId(),
      sheetName
    );
    ...
    ULog.debug('高精度自動マッピング結果 for ' + sheetName + ': ' + JSON.stringify(mapping));
    return mapping;
  } catch (e) {
    console.error('[ERROR]', 'autoMapSheetHeaders エラー: ' + e.message);
    return null;
  }
}
...
function addSpreadsheetUrl(requestUserId, url) {
  ...
  try {
    ...
    const shouldPreservePublishedState = !!(
      configJson.isQuickStart ||
      configJson.setupType === 'quickstart' ||
    ...
    if (shouldPreservePublishedState) {
      ...
      ULog.debug('✅ 外部リソースインポート時に既存の公開状態を保護:', originalPublishedState);
    } else {
      configJson.appPublished = false;
      ULog.debug('🔄 外部リソースインポート - 公開状態をリセット');
    }
    ...
    try {
      ...
      ULog.debug('スプレッドシートからフォーム検出結果:', formResult);

      // 新しい返り値形式に対応
      if (formResult && formResult.success && formResult.formUrl) {
        formUrl = formResult.formUrl;
        configJson.formUrl = formUrl;
        ULog.info('✅ フォーム検出成功 (' + formResult.method + '):', formUrl);
      } else {
        ULog.debug('❌ フォーム検出失敗:', formResult?.message || '不明なエラー');
      }
    } catch (formDetectionError) {
      ULog.warn('フォーム自動検出でエラー:', formDetectionError.message);
    }
    ...
    try {
      ...
      ULog.warn('シートリスト取得でエラー:', sheetError.message);
    ...
    console.error('[ERROR]', 'addSpreadsheetUrl エラー: ' + e.message);
...
function addFormUrl(requestUserId, url) {
  ...
  try {
    ...
    try {
      ...
      // 失敗時は従来通りグローバルのみ更新
      ULog.warn('addFormUrl: シート固有の更新に失敗（グローバルのみ反映）:', cfgErr.message);
      updateUser(currentUserId, { formUrl: url });
    }
    ...
    ULog.info('✅ フォームURL追加完了: ' + currentUserId);
    return createSuccessResponse(null, 'フォームが追加されました。回答ボードでフォームを開くことができるようになりました。');
  } catch (e) {
    console.error('[ERROR]', 'addFormUrl エラー: ' + e.message);
...
function detectFormUrlFromSpreadsheet(spreadsheetId) {
  try {
    ULog.debug('🔍 フォームURL検出開始 (強化版):', spreadsheetId);
    ...
    for (let i = 0; i < sheets.length; i++) {
      ...
      try {
        ...
        if (form) {
          ULog.info('✅ フォーム自動検出成功 (getFormUrl):', form);
          return createSuccessResponse({ formUrl: form, method: 'getFormUrl' });
        }
      } catch (formError) {
    ...
    try {
      ULog.debug('🔍 方法2: 既存フォームからの逆検索を試行中...');
      const foundFormUrl = findFormByDestinationSpreadsheet(spreadsheetId);
      if (foundFormUrl) {
        ULog.info('✅ フォーム逆検索成功:', foundFormUrl);
        return createSuccessResponse({ formUrl: foundFormUrl, method: 'reverseSearch' });
      }
    } catch (reverseError) {
    ...
    // 方法3: 自動フォーム作成（最後の手段）
    try {
      ULog.debug('🔧 方法3: フォーム自動作成を試行中...');
      const createdFormUrl = createFormForSpreadsheet(spreadsheetId, sheets[0].getName());
      if (createdFormUrl) {
        ULog.info('✅ フォーム自動作成成功:', createdFormUrl);
        return createSuccessResponse({ formUrl: createdFormUrl, method: 'autoCreate' });
      }
    } catch (createError) {
      ULog.warn('⚠️ フォーム自動作成エラー:', createError.message);
    }
    ...
    for (let i = 0; i < sheets.length; i++) {
      const sheet = sheets[i];
      try {
        ULog.debug('🔍 方法4: セル内容検索 -', sheet.getName());
        ...
        for (let row = 0; row < values.length; row++) {
          for (let col = 0; col < values[row].length; col++) {
            ...
            if (typeof cellValue === 'string' && cellValue.includes('docs.google.com/forms/')) {
              ...
              const urlMatch = cellValue.match(
                /https:\/\/docs\.google\.com\/forms\/d\/[a-zA-Z0-9_-]+/
              );
              if (urlMatch) {
                ULog.info('✅ フォーム自動検出成功 (セル内容):', urlMatch[0]);
                return createSuccessResponse({ formUrl: urlMatch[0], method: 'cellSearch' });
              }
            }
          }
        }
      } catch (cellError) {
        ULog.warn('⚠️ セル内容検索エラー:', sheet.getName(), cellError.message);
      }
    }

    ULog.debug('❌ 全ての検出方法が失敗しました');
    return createErrorResponse('フォームURLが見つかりませんでした', null, {
      formUrl: null,
      method: 'none',
    });
  } catch (error) {
    console.error('[ERROR]', '❌ detectFormUrlFromSpreadsheet 致命的エラー:', error.message);
    return createErrorResponse(error.message, null, { formUrl: null, method: 'error' });
  }
}
...
function findFormByDestinationSpreadsheet(targetSpreadsheetId) {
  try {
    ULog.debug('🔍 フォーム逆検索開始:', targetSpreadsheetId);
      ...
      try {
        ...
        if (destinationType === FormApp.DestinationType.SPREADSHEET) {
          ...
          if (destinationId && destinationId === targetSpreadsheetId) {
            ...
            ULog.info('✅ 逆検索で一致するフォームを発見:', formUrl);
            return formUrl;
          }
        }
    ...
    ULog.debug('ℹ️ 逆検索完了: 一致するフォームなし (' + checkedCount + '個チェック)');
    return null;
  } catch (error) {
    ULog.warn('❌ フォーム逆検索エラー:', error.message);
    return null;
  }
}
...
function createFormForSpreadsheet(spreadsheetId, sheetName) {
  try {
    ULog.debug('🔧 createFormForSpreadsheet: フォーム自動作成開始', { spreadsheetId, sheetName });

    const spreadsheet = openSpreadsheetOptimized(spreadsheetId);
    const sheet = spreadsheet.getSheetByName(sheetName);

    if (!sheet) {
      console.error('[ERROR]', '❌ 指定されたシートが見つかりません:', sheetName);
      return null;
    }
    ...
    ULog.debug('📝 フォーム作成成功:', form.getEditUrl());
    ...
    headers.forEach(function (header, index) {
      if (header && typeof header === 'string' && header.trim()) {
        ...
        if (headerStr.length > 5) {
          ...
          let item;
          if (
            headerStr.includes('理由') ||
            headerStr.includes('説明') ||
            headerStr.includes('詳細')
          ) {
            // 長文回答用
          ...
          questionCount++;
          ULog.debug(`✅ 質問項目追加: ${headerStr}`);
        }
      }
    });

    if (questionCount === 0) {
      ULog.warn('⚠️ 有効な質問項目が見つかりませんでした');
      return null;
    }
    ...
    try {
      ...
      ULog.info('✅ フォーム回答先をスプレッドシートに設定');
    } catch (destError) {
      ULog.warn('⚠️ 回答先設定でエラー:', destError.message);
    }

    const formUrl = form.getPublishedUrl();
    ULog.info('✅ フォーム自動作成完了:', formUrl);
    return formUrl;
  } catch (error) {
    console.error('[ERROR]', '❌ createFormForSpreadsheet エラー:', error.message);
    return null;
  }
}
...
function unpublishBoard(requestUserId) {
  ULog.debug('🔧 unpublishBoard: 公開停止処理開始', { requestUserId });
  verifyUserAccess(requestUserId);
  try {
    const currentUserId = requestUserId; // requestUserId を使用
    ...
    const userInfo = findUserById(currentUserId);
    if (!userInfo) {
      console.error('[ERROR]', '❌ ユーザー情報が見つかりません:', currentUserId);
      throw new Error('ユーザー情報が見つかりません。');
    }

    ULog.info('✅ ユーザー情報取得成功:', {
    ...
    ULog.debug('🧹 公開停止: 設定を完全クリア完了（スプレッドシート情報も含む）');
    ...
    ULog.debug('🔧 データベース更新データ:', updateData);
    updateUser(currentUserId, updateData);
    ULog.debug('🔧 データベース更新完了');
    ...
    try {
      ...
      if (typeof getUnifiedExecutionCache === 'function') {
        ...
        if (cache && typeof cache.clearUserInfo === 'function') {
          cache.clearUserInfo();
          ULog.debug('🧹 UnifiedCache cleared after unpublish');
        }
      }
      ...
      if (typeof CacheService !== 'undefined') {
        try {
          ...
          ULog.debug('🧹 CacheService cleared after unpublish');
        } catch (cacheServiceError) {
          ULog.debug('CacheService clear 警告:', cacheServiceError.message);
        }
      }
    } catch (cacheError) {
      ULog.warn('キャッシュ無効化でエラーが発生しましたが、処理を続行します:', cacheError.message);
    }

    ULog.debug('回答ボードの公開を正常に停止しました - ユーザーID:', currentUserId);
    ...
    console.error('[ERROR]', 'unpublishBoard エラー: ' + e.message);
...
function setActiveSheet(requestUserId, sheetName) {
  ULog.debug('🎯 setActiveSheet: アクティブシート設定開始', { requestUserId, sheetName });
  verifyUserAccess(requestUserId);
  ...
  try {
    ...
    ULog.info('✅ setActiveSheet: アクティブシート設定完了', { sheetName });

    return createSuccessResponse({
      activeSheetName: sheetName,
      timestamp: new Date().toISOString(),
    }, `シート「${sheetName}」をアクティブに設定しました`);
  } catch (e) {
    console.error('[ERROR]', 'setActiveSheet エラー: ' + e.message);
...
function setDisplayOptions(requestUserId, displayOptions, options = {}) {
  ...
  try {
    ...
    ULog.debug('setDisplayOptions: 設定更新', displayOptions);
    ...
    console.error('[ERROR]', 'setDisplayOptions エラー: ' + e.message);
...
function createBoardFromAdmin(requestUserId) {
  ...
  try {
    ...
    console.error('[ERROR]', 'createBoardFromAdmin エラー: ' + e.message);
...
function getExistingBoard(requestUserId) {
  ...
  try {
    ...
    console.error('[ERROR]', 'getExistingBoard エラー: ' + e.message);
    return createErrorResponse('既存ボード情報の取得に失敗しました: ' + e.message, null, null);
  }
}
  ...
  try {
    ...
    if (email) {
      ...
      if (domainInfo.deployDomain && domainInfo.deployDomain !== '' && !domainInfo.isDomainMatch) {
        ULog.warn('Domain access denied:', domainInfo.currentDomain, 'vs', domainInfo.deployDomain);
        return {
          authenticated: false,
          email: null,
    ...
    console.error('[ERROR]', 'verifyUserAuthentication エラー: ' + e.message);
...
function getSheetDetails(requestUserId, spreadsheetId, sheetName) {
  // パラメータ型検証（従来版）
  ULog.debug('🔍 getSheetDetails (userId version) parameter validation:', {
  ...
  if (typeof requestUserId === 'object') {
    console.error(
      '[ERROR]',
      '❌ CRITICAL: getSheetDetails received object as requestUserId parameter'
    );
    console.error(
      '[ERROR]',
      '❌ This suggests you meant to call getSheetDetailsFromContext instead'
    );
  ...
  if (typeof requestUserId !== 'string' || requestUserId.trim() === '') {
    console.error('[ERROR]', '❌ CRITICAL: Invalid requestUserId parameter:', {
      requestUserId,
  ...
  if (spreadsheetId !== null && spreadsheetId !== undefined) {
    if (typeof spreadsheetId !== 'string') {
      console.error('[ERROR]', '❌ CRITICAL: Invalid spreadsheetId parameter type:', {
        spreadsheetId,
    ...
    if (spreadsheetId.length > 0 && !/^[a-zA-Z0-9_-]{10,100}$/.test(spreadsheetId)) {
      console.error('[ERROR]', '❌ CRITICAL: Invalid spreadsheetId format:', {
        spreadsheetId,
  ...
  try {
    ...
    if (!sheetName || (typeof sheetName === 'string' && sheetName.trim() === '')) {
      ULog.warn('⚠️ getSheetDetails: sheetNameが空です。フォールバック処理を実行中...');
      ...
      try {
        ...
        if (userInfo && userInfo.configJson) {
          ...
          if (
            fallbackSheetName &&
            typeof fallbackSheetName === 'string' &&
            fallbackSheetName.trim() !== ''
          ) {
            ULog.info('✅ configJsonからフォールバックシート名を使用:', fallbackSheetName);
            sheetName = fallbackSheetName;
          } else {
            throw new Error(
              'getSheetDetails: sheetNameが必須ですが、configJsonからも取得できませんでした'
            );
          }
        } else {
          throw new Error(
            'getSheetDetails: sheetNameが必須ですが、ユーザー情報からも取得できませんでした'
          );
        }
      } catch (fallbackError) {
        console.error('[ERROR]', '❌ フォールバック処理に失敗:', fallbackError.message);
    ...
    if (typeof targetId === 'string' && !/^[a-zA-Z0-9_-]{10,100}$/.test(targetId)) {
      console.error(
        '[ERROR]',
        '❌ CRITICAL: targetId appears to be a sheet name instead of spreadsheet ID:',
        {
          targetId,
          sheetName,
      ...
      throw new Error(
        `無効なspreadsheetID形式です: "${targetId}" はシート名のようです。パラメータの順序を確認してください。`
      );
    }
    // SpreadsheetApp.openById()の代わりにSheets APIを使用（権限問題回避）
    ULog.debug('🔧 Sheets APIを使用してヘッダーを取得中:', { targetId, sheetName });
    ...
    try {
      ...
      if (batch && batch.valueRanges && batch.valueRanges[0] && batch.valueRanges[0].values) {
        ...
        ULog.info('✅ Sheets APIでヘッダー取得成功:', headers.length, '列');
      } else {
        ULog.warn('⚠️ ヘッダー行が空または取得できませんでした');
        headers = [];
      }
    } catch (apiError) {
      console.error('[ERROR]', '❌ Sheets API取得エラー:', apiError.message);

      // フォールバック: SpreadsheetApp.openById()を試行（権限がある場合のみ）
      ULog.warn('⚠️ フォールバック: SpreadsheetApp.openById()を試行');
      try {
        ...
        ULog.info('✅ SpreadsheetApp フォールバック成功');
      } catch (spreadsheetError) {
        console.error(
          '[ERROR]',
          '❌ SpreadsheetApp フォールバックも失敗:',
          spreadsheetError.message
        );
        throw new Error(
          `ヘッダー取得に失敗しました。Sheets API: ${apiError.message}, SpreadsheetApp: ${spreadsheetError.message}`
        );
      }
    }
    ...
    try {
      ...
      ULog.warn('getConfig failed in getSheetDetails:', e.message);
    }
    ...
    console.error('[ERROR]', 'getSheetDetails error:', error.message);
...
function createExecutionContext(requestUserId, options = {}) {
  ...
  ULog.debug('🚀 ExecutionContext作成開始: userId=%s', requestUserId);

  try {
    // 1. 共有リソースの取得（最適化：既存リソース再利用対応）
    const originalSheetsService = options.reuseService || getSheetsServiceCached();
    let userInfo =
      options.reuseUserInfo ||
      getOrFetchUserInfo(requestUserId, 'userId', {
        useExecutionCache: true,
        ttl: 300,
      });

    if (!userInfo) {
      ULog.warn('getOrFetchUserInfo miss: userId=%s', requestUserId);
      userInfo = findUserByIdFresh(requestUserId);

      if (userInfo) {
        _executionUserInfoCache = { userId: requestUserId, userInfo };
        ULog.info('✅ findUserByIdFresh success: cached for execution');
      } else {
        console.error('[ERROR]', '❌ findUserByIdFresh failed for userId=%s', requestUserId);
        throw new Error(
          'ユーザー情報が取得できません。ユーザー登録が完了しているか、データベース設定を確認してください'
        );
      }
    }
    ...
    if (
      !originalSheetsService ||
      !originalSheetsService.baseUrl ||
      !originalSheetsService.accessToken
    ) {
      throw new Error(
        'SheetsServiceオブジェクトが無効です。baseUrlまたはaccessTokenが見つかりません'
      );
    }
    ...
    if (!sheetsService || !sheetsService.baseUrl || !sheetsService.accessToken) {
      console.error('[ERROR]', '❌ SheetsService作成失敗、オリジナルサービスを使用');
      // 最後の手段：オリジナルサービスを直接使用（参照の問題は発生するが機能する）
      sheetsService = originalSheetsService;
    }

    ULog.debug('DEBUG: SheetsService安全コピー完了 - baseUrl存在:', !!sheetsService.baseUrl);
    ...
    const context = {
      ...
      userInfo: Object.assign({}, userInfo), // 浅いコピー（userInfoは単純オブジェクト）
    ...
    ULog.debug('DEBUG: context.sheetsService initialized successfully');

    const endTime = new Date().getTime();
    ULog.info(
      '✅ ExecutionContext作成完了: %dms (リソース再利用: Service=%s, UserInfo=%s)',
      endTime - startTime,
      !!options.reuseService,
      !!options.reuseUserInfo
    );

    return context;
  } catch (error) {
    console.error('[ERROR]', '❌ ExecutionContext作成エラー:', error.message);
    throw new Error('実行コンテキストの初期化に失敗しました: ' + error.message);
  }
}

/**
 * @deprecated この関数は削除予定です。database.gsのupdateUser()を使用してください。
 * 最適化版updateUser（インメモリ更新）
 * @param {object} context - 実行コンテキスト
 * @param {object} updateData - 更新データ
 */
function updateUserOptimized(context, updateData) {
  ULog.debug('💾 updateUserOptimized: 変更をコンテキストに蓄積');
  ...
  ULog.debug('📊 蓄積された変更数: %d', Object.keys(context.pendingUpdates).length);
}
...
function commitAllChanges(context) {
  const startTime = new Date().getTime();
  ULog.debug('💽 commitAllChanges: 一括DB書き込み開始');

  if (!context.hasChanges || Object.keys(context.pendingUpdates).length === 0) {
    ULog.debug('📝 変更なし: DB書き込みをスキップ');
    return;
  }

  try {
    // Phase 1: DB更新実行 - UnifiedBatchProcessor活用
    ULog.debug('⚡ UnifiedBatchProcessor使用でDB更新を最適化');
    ...
    try {
      ULog.debug('🗑️ トランザクション後のキャッシュ無効化開始');

      // 1. ユーザー関連キャッシュの無効化
      invalidateUserCacheTransaction(
      ...
      ULog.info('✅ キャッシュ無効化完了');
    } catch (cacheError) {
      // キャッシュエラーは致命的ではないが、ログに記録
      ULog.warn('⚠️ キャッシュ無効化で軽微なエラー:', cacheError.message);
    }

    const endTime = new Date().getTime();
    ULog.info(
      '✅ 一括DB書き込み+キャッシュ無効化完了: %dms, 変更項目数: %d',
      endTime - startTime,
      Object.keys(context.pendingUpdates).length
    );
    ...
    console.error('[ERROR]', '❌ 一括DB書き込みエラー:', error.message);
    ...
    if (typeof cacheManager !== 'undefined' && cacheManager.remove) {
      ...
      userCacheKeys.forEach((key) => {
        try {
          ...
          ULog.warn('キャッシュキー削除エラー:', key, keyError.message);
        }
      });
      ...
      try {
        ...
        ULog.warn(
          'invalidateUserCacheTransaction: user-scoped pattern clear failed:',
          patternErr.message
        );
      }
      ...
      if (spreadsheetId) {
        try {
          ...
          ULog.warn('パターンベースキャッシュ削除エラー:', patternError.message);
        }
      }
    }

    ULog.info('✅ トランザクション用キャッシュ無効化完了');
  } catch (error) {
    ULog.warn('⚠️ トランザクション用キャッシュ無効化エラー:', error.message);
...
function buildResponseFromContext(context) {
  const startTime = new Date().getTime();
  ULog.debug('🏗️ buildResponseFromContext: DB検索なしでレスポンス構築');

  // 🚨 緊急診断: buildResponseFromContext受信時のcontext確認
  ULog.debug('🚨 CRITICAL: Context at buildResponseFromContext entry:', {
  ...
  if (typeof context === 'string') {
    console.error(
      '[ERROR]',
      '❌ CRITICAL: buildResponseFromContext received string context, attempting recovery'
    );
    try {
      context = JSON.parse(context);
      ULog.info('✅ Successfully recovered context from JSON string');

      // sheetsServiceが失われている場合は再作成
      if (!context.sheetsService) {
        ULog.warn('⚠️ SheetsService lost during serialization, recreating...');
        const accessToken = getServiceAccountTokenCached();
        if (accessToken) {
          context.sheetsService = createSheetsService(accessToken);
          ULog.info('✅ SheetsService recreated after deserialization');
        }
      }
    } catch (parseError) {
      console.error('[ERROR]', '❌ Failed to recover context from JSON:', parseError.message);
  ...
  try {
    ...
    if (configJson.publishedSheetName) {
      if (typeof configJson.publishedSheetName === 'string') {
        ...
        console.error(
          '[ERROR]',
          '❌ publishedSheetNameが不正な型です:',
          typeof configJson.publishedSheetName,
          configJson.publishedSheetName
        );
        ULog.debug('🔧 publishedSheetNameを空文字にリセットしました');
        publishedSheetName = '';
        // 不正な値をデータベースから修正する必要があることをログ出力
        ULog.warn(
          '⚠️ データベースの publishedSheetName を修正してください。現在の値:',
          configJson.publishedSheetName
        );
      }
    }
    ...
    const response = {
      ...
      appUrls: (function () {
        try {
          ...
          ULog.warn('generateUserUrls失敗、フォールバック使用:', e.message);
          ...
          return {
            ...
            viewUrl:
              ...
              fallbackBaseUrl +
                '?userId=' +
                encodeURIComponent(context.requestUserId) +
                '&mode=view',
            setupUrl:
              fallbackBaseUrl + '?setup=true&userId=' + encodeURIComponent(context.requestUserId),
      ...
      boardUrl: (function () {
        try {
          ...
          return (
            urls.viewUrl ||
            userInfo.viewUrl ||
            getWebAppUrl() + '?userId=' + encodeURIComponent(context.requestUserId) + '&mode=view'
          );
        } catch (e) {
          ULog.warn('boardUrl生成失敗、フォールバック使用:', e.message);
          return (
            userInfo.viewUrl ||
            getWebAppUrl() + '?userId=' + encodeURIComponent(context.requestUserId) + '&mode=view'
          );
      ...
      viewUrl: (function () {
        try {
          ...
          ULog.warn('viewUrl生成失敗、フォールバック使用:', e.message);
          return (
            userInfo.viewUrl ||
            getWebAppUrl() + '?userId=' + encodeURIComponent(context.requestUserId) + '&mode=view'
          );
      ...
      webAppUrl: (function () {
        try {
          ...
          ULog.warn('webAppUrl生成失敗、フォールバック使用:', e.message);
          return getWebAppUrl();
        }
      })(),
    ...
    if (spreadsheetId && publishedSheetName) {
      try {
        ...
        if (publishedSheetName === 'true' || publishedSheetName === 'false') {
          console.error(
            '[ERROR]',
            '❌ buildResponseFromContext: 無効なシート名を検出:',
            publishedSheetName
          );
          ULog.warn('⚠️ シート詳細取得をスキップします');
        } else {
          ULog.debug('DEBUG: Calling getSheetDetailsFromContext with context service');
        ...
        ULog.warn('buildResponseFromContext: シート詳細取得エラー（基本情報のみで継続）:', e.message);
        response.allSheets = [];
        response.sheetNames = [];
        response.headers = [];
    ...
    const endTime = new Date().getTime();
    ULog.info('✅ レスポンス構築完了: %dms', endTime - startTime);

    return response;
  } catch (error) {
    console.error('[ERROR]', '❌ buildResponseFromContext エラー:', error.message);
...
function getSheetDetailsFromContext(context, spreadsheetId, sheetName) {
  ULog.debug('DEBUG: getSheetDetailsFromContext called with context object');

  // 🚨 パラメータ型検証: 正しい型で呼ばれているか確認
  ULog.debug('🔍 Parameter validation:', {
    contextType: typeof context,
    spreadsheetIdType: typeof spreadsheetId,
    sheetNameType: typeof sheetName,
  ...
  if (typeof context === 'string') {
    console.error(
      '[ERROR]',
      '❌ CRITICAL: getSheetDetailsFromContext received string as context parameter'
    );
    console.error(
      '[ERROR]',
      '❌ This suggests parameter order mismatch. Expected: (context:object, spreadsheetId:string, sheetName:string)'
    );
    console.error('[ERROR]', '❌ Received parameters:', {
      param1: typeof context === 'string' ? context.substring(0, 50) : context,
      param2: spreadsheetId,
      param3: sheetName,
    });
    throw new Error(
      'Parameter order mismatch: context must be an object, not a string. Check function call parameters.'
    );
  }
      ...
      contextUserInfo:
        context && context.userInfo
          ? {
              ...
              publishedSheetName: JSON.parse(context.userInfo.configJson || '{}')
                .publishedSheetName,
            }
          : 'no userInfo',
    });
    ...
    if (context && context.userInfo && context.userInfo.configJson) {
      try {
        ...
        if (
          ...
          ULog.warn('⚠️ Using fallback publishedSheetName from config:', fallbackSheetName);
          sheetName = fallbackSheetName;
        } else {
          throw new Error(
            'getSheetDetailsFromContext: sheetName parameter must be a valid non-empty string. Received: ' +
              sheetName
          );
        }
      } catch (parseError) {
        throw new Error(
          'getSheetDetailsFromContext: sheetName parameter must be a valid non-empty string and configJson fallback failed'
        );
      }
    } else {
      throw new Error(
        'getSheetDetailsFromContext: sheetName parameter must be a valid non-empty string and no fallback available'
      );
    }
  }
  ...
  try {
    ...
    if (!context?.sheetsService?.baseUrl || !context?.sheetsService?.accessToken) {
      ULog.warn('⚠️ ExecutionContextのSheetsServiceが無効、復旧中...');
      try {
        // Step 1: 新しいサービスを取得（代入前の検証）
        ULog.debug('🔍 Step 1: getSheetsServiceCached呼び出し前');
        const newService = getSheetsServiceCached();
        ULog.debug('🔍 Step 2: getSheetsServiceCached返り値の検証:', {
        ...
        ULog.debug('🔍 Step 3: 代入前のコンテキスト状態:', {
          hasContext: !!context,
          contextType: typeof context,
        ...
        // Step 3: 実際の代入
        ULog.debug('🔍 Step 4: サービス代入実行');
        context.sheetsService = newService;
        ULog.debug('🔍 Step 5: 代入直後の確認');

        // Step 4: 代入後の検証
        ULog.debug('🔍 復旧後のサービスオブジェクト検証:', {
        ...
        if (
          !context.sheetsService ||
          !context.sheetsService.baseUrl ||
          !context.sheetsService.accessToken
        ) {
          ULog.warn('⚠️ 代入が失敗、直接サービス作成でリトライ');
          ...
          if (
            ...
            throw new Error(
              'SheetsService復旧に失敗: 直接作成でも有効なサービスオブジェクトを作成できません'
            );
          }

          ULog.info('✅ SheetsService直接作成による復旧完了');
        } else {
          ULog.info('✅ SheetsService復旧完了');
        }
      } catch (serviceError) {
        console.error('[ERROR]', '❌ SheetsService復旧エラー:', serviceError.message);
        console.error('[ERROR]', '❌ Error stack:', serviceError.stack);

        // 最後の手段: 強制リフレッシュでリトライ
        try {
          ULog.warn('🔄 最後の手段: 強制リフレッシュでサービス復旧をリトライ');
          ...
          ULog.info('✅ 強制リフレッシュによるSheetsService復旧成功');
        } catch (retryError) {
          console.error('[ERROR]', '❌ 強制リフレッシュリトライも失敗:', retryError.message);
          throw new Error(
            `Sheets APIサービスの復旧に完全に失敗しました。初期エラー: ${serviceError.message}, リトライエラー: ${retryError.message}`
          );
        }
      }
    }
    ...
    ULog.debug('DEBUG: Calling getSpreadsheetsData with context service');

    // 最終的なサービス状態の検証（API呼び出し前）
    ULog.debug('🔍 API呼び出し前の最終サービス検証:', {
      hasService: !!context.sheetsService,
      hasBaseUrl: !!(context.sheetsService && context.sheetsService.baseUrl),
      hasAccessToken: !!(context.sheetsService && context.sheetsService.accessToken),
      ...
      hasGet: !!(
        context.sheetsService &&
        context.sheetsService.spreadsheets &&
        typeof context.sheetsService.spreadsheets.get === 'function'
      ),
      serviceType: typeof context.sheetsService,
      baseUrl: context.sheetsService && context.sheetsService.baseUrl,
    });
    ...
    try {
      ...
      ULog.debug('DEBUG: getSpreadsheetsData success, sheets count:', data.sheets?.length || 0);
    } catch (apiError) {
      console.error('[ERROR]', '❌ getSpreadsheetsData failed:', apiError.message);
    ...
    if (!targetSheet) {
      throw new Error(
        `指定されたシート '${sheetName}' が見つかりません。利用可能なシート: ${data.sheets.map((s) => s.properties.title).join(', ')}`
      );
    }

    // ヘッダー行をAPIで取得
    const range = `'${sheetName}'!1:1`;
    ULog.debug('DEBUG: Fetching headers with range:', range);
    let batch;
    try {
      ...
      ULog.debug('DEBUG: batchGetSheetsData success, valueRanges count:', batch.valueRanges.length);
    } catch (batchError) {
      console.error('[ERROR]', '❌ batchGetSheetsData failed:', batchError.message);
      throw new Error('ヘッダー行の取得に失敗: ' + batchError.message);
    }

    const headers =
      batch.valueRanges && batch.valueRanges[0] && batch.valueRanges[0].values
        ? batch.valueRanges[0].values[0] || []
        : [];

    ULog.debug('DEBUG: Retrieved headers:', headers.length, 'columns');
    ...
    ULog.info('✅ getSheetDetails success:', {
    ...
    console.error('[ERROR]', '❌ getSheetDetails エラー:', error.message);
...
function getConfigFromContext(context, sheetName) {
  try {
    ...
    ULog.warn('getConfigFromContext エラー:', e.message);
    return {};
  }
}
...
function saveSheetConfigInContext(context, spreadsheetId, sheetName, config) {
  ULog.debug('💾 saveSheetConfigInContext: インメモリ更新');
  ...
  try {
    ...
    try {
      ...
      if (Object.keys(topLevelUpdates).length > 0) {
        ...
        ULog.debug('🔗 同期: トップレベルのヘッダー設定を更新', topLevelUpdates);
      }
    } catch (syncErr) {
      ULog.warn('saveSheetConfigInContext: トップレベル同期で警告（処理継続）:', syncErr.message);
    }
    ...
    ULog.info('✅ シート設定をコンテキストに保存: %s', sheetKey);
  } catch (error) {
    console.error('[ERROR]', '❌ saveSheetConfigInContext エラー:', error.message);
...
function switchToSheetInContext(context, spreadsheetId, sheetName) {
  ULog.debug('🔄 switchToSheetInContext: インメモリ更新');
  ...
  try {
    ...
    try {
      ...
      if (!sheetCfg.formUrl) {
          ...
          if (targetSheet) {
            try {
              ...
              if (url) {
                ...
                ULog.info('✅ switchToSheetInContext: シートからフォームURLを直接取得:', url);
              }
            } catch (getFormErr) {
              ULog.debug('getFormUrl 未取得（例外）:', getFormErr.message);
            }
          }
        } catch (sheetOpenErr) {
          ULog.debug('シートアクセス失敗（フォームURL検出スキップ）:', sheetOpenErr.message);
        }
        ...
        if (!sheetCfg.formUrl) {
          ...
          if (detected && detected.success && detected.formUrl) {
            ...
            ULog.info(
              '✅ switchToSheetInContext: フォールバック検出でフォームURL設定:',
              detected.formUrl
            );
          } else {
            ULog.warn('⚠️ シート固有のフォームURLが見つかりません（検出不可）:', {
              シート名: sheetName,
              設定キー: sheetKey,
      ...
      ULog.warn('switchToSheetInContext: フォームURL同期で警告:', formSyncErr.message);
    }
    ...
    ULog.info('✅ シート切り替えをコンテキストに保存: %s', sheetName);
  } catch (error) {
    console.error('[ERROR]', '❌ switchToSheetInContext エラー:', error.message);
...
function syncFormUrlForActiveSheet(configJson, sheetName) {
  if (!sheetName || !configJson) {
    ULog.warn('⚠️ syncFormUrlForActiveSheet: 無効なパラメータ');
    return;
  }
  ...
  if (sheetConfig && typeof sheetConfig === 'object' && sheetConfig.formUrl) {
    ...
    ULog.debug('🔗 フォームURL同期完了:', {
      シート名: sheetName,
      旧URL: oldFormUrl,
      新URL: configJson.formUrl,
    });
  } else {
    ULog.warn('⚠️ シート固有のフォームURLが見つかりません:', {
      シート名: sheetName,
      設定キー: sheetConfigKey,
...
function setDisplayOptionsInContext(context, displayOptions) {
  ULog.debug('🎛️ setDisplayOptionsInContext: インメモリ更新');
  ...
  try {
    ...
    ULog.info('✅ 表示オプションをコンテキストに保存:', displayOptions);
  } catch (error) {
    console.error('[ERROR]', '❌ setDisplayOptionsInContext エラー:', error.message);
...
function saveAndPublish(requestUserId, sheetName, config) {
  ...
  return executeWithStandardizedLock('BATCH_OPERATION', 'saveAndPublish', () => {
    ULog.debug('🚀 saveAndPublishOptimized開始: sheetName=%s', sheetName);
    ...
    ULog.info('✅ Phase 2完了: 全設定をコンテキストに蓄積');

    // Phase 2.5: 削除 - 重複キャッシュクリアを削除（Phase 3.5で実行）
    ULog.debug('⚡ 最適化: DB書き込み前のキャッシュクリアを省略（重複削除）');

    // Phase 3: 一括DB書き込み（1回のみ）- エラー処理強化
    ULog.debug('💽 Phase 3: 一括DB書き込み開始（回復力のある実行）');
    ...
    while (dbWriteAttempts < maxDbRetries) {
      try {
        dbWriteAttempts++;
        ULog.debug(`📊 DB書き込み試行 ${dbWriteAttempts}/${maxDbRetries}`);

        commitAllChanges(context);
        ULog.info('✅ Phase 3完了: DB書き込み完了（回復力のある実行）');
        break; // 成功時はループを抜ける
      } catch (dbError) {
        console.error('[ERROR]', `❌ DB書き込み失敗 (試行${dbWriteAttempts}):`, dbError.message);

        if (dbWriteAttempts >= maxDbRetries) {
          console.error('[ERROR]', '❌ Phase 3エラー: 最大リトライ回数に達しました');
        ...
        if (dbError.message.includes('503') || dbError.message.includes('429')) {
          ...
          ULog.warn(`⏳ ${retryDelay}ms待機後にリトライします...`);
    ...
    ULog.debug('🔥 UnifiedExecutionCache使用でキャッシュウォーミング最適化中...');
    try {
      ...
      if (freshUserInfo) {
        ...
        ULog.info('✅ UnifiedExecutionCache統合でキャッシュウォーミング完了');
      }
    } catch (warmingError) {
      ULog.warn(
        '⚠️ 最適化キャッシュウォーミングでエラー（フォールバック実行）:',
        warmingError.message
      );
      ...
      try {
        ...
        if (freshUserInfo) {
          context.userInfo = freshUserInfo;
          ULog.info('✅ フォールバック方式でキャッシュウォーミング完了');
        }
      } catch (fallbackError) {
        ULog.warn(
          '⚠️ フォールバックキャッシュウォーミングも失敗（処理続行）:',
    ...
    ULog.debug('🏗️ Phase 4: レスポンス構築開始');

    // 🚨 緊急診断: buildResponseFromContext呼び出し前のcontext検証
    ULog.debug('🚨 CRITICAL: Context before buildResponseFromContext:', {
      contextType: typeof context,
      isString: typeof context === 'string',
      isObject: typeof context === 'object' && context !== null,
      hasUserInfo: context && typeof context.userInfo !== 'undefined',
    ...
    ULog.info('✅ Phase 4完了: レスポンス構築完了');

    // パフォーマンス統計
    const totalTime = new Date().getTime() - overallStartTime;
    ULog.debug('📊 saveAndPublishOptimized パフォーマンス統計:');
    ULog.debug('  ⏱️ 総実行時間: %dms', totalTime);
    ULog.debug('  🔧 SheetsService作成: %d回', context.stats.sheetsServiceCreations);
    ULog.debug('  🗄️ DB検索: %d回', context.stats.dbQueries);
    ULog.debug('  ⚡ 操作回数: %d回', context.stats.operationsCount);
...
function resetConfigJson(requestUserId) {
  ...
  ULog.debug('🔄 ConfigJsonリセット開始 for user:', requestUserId);
  ...
  try {
    ...
    console.error('[ERROR]', '❌ ConfigJsonリセットエラー:', error.message);
...
function syncConfigurationState(requestUserId, newConfig, flowType) {
  try {
    ...
    // データ型の詳細検証とデバッグログ
    ULog.debug('syncConfigurationState: 受信データ検証', {
    ...
    if (!newConfig) {
      ...
      return createErrorResponse([error], null, null);
    }

    // 配列として送信された場合の処理
    if (Array.isArray(newConfig)) {
      ULog.debug('syncConfigurationState: 配列形式の設定データを検出、オブジェクト変換を試行');
      try {
        // 配列の最初の要素がオブジェクトの場合はそれを使用
        if (newConfig.length > 0 && typeof newConfig[0] === 'object') {
          newConfig = newConfig[0];
          ULog.debug('syncConfigurationState: 配列の最初の要素を設定データとして使用');
        } else {
    ...
    ULog.debug('syncConfigurationState: 設定同期開始', {
      userId: requestUserId,
      flowType: flowType,
    ...
    if (flowType === 'quickstart' && mergedConfig.appPublished === true) {
      ULog.debug('syncConfigurationState: クイックスタートフロー - validation緩和モード');
      skipStrictValidation = true;
      // シート設定が含まれていることを確認
      if (sheetConfigKeys.length > 0) {
        ULog.debug('syncConfigurationState: シート設定キーを検出、保存を続行', sheetConfigKeys);
      }
    } else if (flowType === 'repair' || flowType === 'custom') {
      ULog.debug('syncConfigurationState: 修復/カスタムフロー - 部分的validation適用');
      skipStrictValidation = true;
      // 修復フローでは既存の公開状態を考慮
      if (mergedConfig.appPublished || userInfo.configJson?.includes('appPublished":true')) {
        ULog.debug('syncConfigurationState: 公開済み状態のためvalidationをバイパス');
        const warnings = ['公開済み状態のためvalidationをバイパスしました'];
        ULog.warn('⚠️ Auto-healing後の警告:', warnings);
      }
    }
    ...
    if (!skipStrictValidation) {
      ...
      if (!validation.isValid) {
        logError(
          `設定検証失敗: ${validation.errors.join(', ')}`,
          'syncConfigurationState',
          UNIFIED_CONSTANTS.ERROR.SEVERITY.MEDIUM,
          UNIFIED_CONSTANTS.ERROR.CATEGORIES.VALIDATION
        );
        return createErrorResponse(validation.errors, null, {
          warnings: validation.warnings || [],
          context: { flowType, userId: requestUserId },
        });
      }
    } else {
      ULog.debug('syncConfigurationState: 厳格なvalidationをスキップ (フロー:', flowType, ')');
    }
    ...
    ULog.debug('syncConfigurationState: 設定同期完了', { userId: requestUserId, flowType: flowType });
    return createSuccessResponse({ flowType: flowType || '', configJson: mergedConfig });
  } catch (error) {
    logError(
      error,
      'syncConfigurationState',
    ...
    return createErrorResponse(['設定の同期中にシステムエラーが発生しました'], null, {
...
function getApplicationEnabled() {
  try {
    ...
    console.error('[ERROR]', 'getApplicationEnabled エラー:', error.message);
    // エラー時はデフォルトで有効とする
    return true;
  }
}
...
function setApplicationEnabled(enabled) {
  try {
    ...
    ULog.debug('setApplicationEnabled:', message);
    ...
    console.error('[ERROR]', 'setApplicationEnabled エラー:', error.message);
...
function checkApplicationAccess() {
  try {
    ...
    console.error('[ERROR]', 'checkApplicationAccess エラー:', error.message);
    return {
      hasAccess: false,
...
function schedulePeriodicCleanup() {
  try {
    ...
    ULog.debug('📅 定期クリーンアップをスケジュール');
  } catch (error) {
    ULog.warn('クリーンアップスケジュールエラー:', error.message);
  }
}
