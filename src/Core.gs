...
function registerNewUser(adminEmail) {
  ...
  var existingUser = DB.findUserByEmail(adminEmail);
  var userId, appUrls;
...
function addReaction(requestUserId, rowIndex, reactionKey, sheetName) {
  ...
  try {
    ...
    var boardOwnerInfo = DB.findUserById(ownerUserId);
    if (!boardOwnerInfo) {
...
function executeGetPublishedSheetData(requestUserId, classFilter, sortOrder, adminMode) {
    ...
    console.log('getPublishedSheetData: isOwner=%s, ownerId=%s, currentUserId=%s', isOwner, configJson.ownerId, currentUserId);

    // データ取得
    var sheetData = getSheetData(currentUserId, publishedSheetName, classFilter, sortOrder, adminMode);
    console.log('getPublishedSheetData: sheetData status=%s, totalCount=%s', sheetData.status, sheetData.totalCount);
    ...
    console.log('getPublishedSheetData: Configured Headers - mainHeaderName=%s, reasonHeaderName=%s, classHeaderName=%s, nameHeaderName=%s', mainHeaderName, reasonHeaderName, classHeaderName, nameHeaderName);
...
function getSheetsList(userId) {
  try {
    ...
    console.log('✅ getSheetsList: Found sheets for userId %s: %s', userId, JSON.stringify(sheetList));
    return sheetList;
  } catch (e) {
...
function refreshBoardData(requestUserId) {
  ...
  try {
    ...
    var userInfo = DB.findUserById(currentUserId);
    if (!userInfo) {
      throw new Error('ユーザー情報が見つかりません');
    }
...
function formatSheetDataForFrontend(rawData, mappedIndices, headerIndices, adminMode, isOwner, displayMode) {
  ...
  return rawData.map(function(row, index) {
    ...
    console.log('formatSheetDataForFrontend: Row %s - classIndex=%s, opinionIndex=%s, reasonIndex=%s, nameIndex=%s', index, classIndex, opinionIndex, reasonIndex, nameIndex);
    console.log('formatSheetDataForFrontend: Row data length=%s, first few values=%s', row.originalData ? row.originalData.length : 'undefined', row.originalData ? JSON.stringify(row.originalData.slice(0, 5)) : 'undefined');
...
function switchToSheet(userId, spreadsheetId, sheetName, options = {}) {
  try {
    ...
    var userInfo = options.userInfo || DB.findUserById(currentUserId);
    if (!userInfo) {
      throw new Error('ユーザー情報が見つかりません');
    }
...
function generateQuickStartResponse(setupContext, createdFiles, updatedConfig) {
  ...
  console.log('✅ クイックスタートセットアップ完了: ' + requestUserId);
...
function initializeQuickStartContext(requestUserId) {
  ...
  // ユーザー情報の取得
  var userInfo = DB.findUserById(requestUserId);
  if (!userInfo) {
    throw new Error('ユーザー情報が見つかりません');
  }
...
function getHeaderIndices(spreadsheetId, sheetName) {
  console.log('getHeaderIndices received in core.gs: spreadsheetId=%s, sheetName=%s', spreadsheetId, sheetName);
...
function processReaction(spreadsheetId, sheetName, rowIndex, reactionKey, reactingUserEmail) {
  try {
    ...
    try {
      ...
      console.log('リアクション切り替え完了: ' + reactingUserEmail + ' → ' + reactionKey + ' (' + userAction + ')', {
...
function createLinkedSpreadsheet(userEmail, form, dateTimeString) {
  try {
    ...
    try {
      ...
      console.log('スプレッドシート作成は完了しました。サービスアカウント共有は後で設定してください。');
    }
...
function executeGetSheetData(userId, sheetName, classFilter, sortMode) {
    ...
    console.log('getSheetData: isOwner=%s, ownerId=%s, userId=%s', isOwner, configJson.ownerId, userId);
...
function mapConfigToActualHeaders(configHeaders, actualHeaderIndices) {
  ...
  console.log('mapConfigToActualHeaders: Available headers in spreadsheet: %s', JSON.stringify(availableHeaders));
  ...
  for (var configKey in configHeaders) {
    ...
    if (configHeaderName && actualHeaderIndices.hasOwnProperty(configHeaderName)) {
      ...
      console.log('mapConfigToActualHeaders: Exact match found for %s: index %s', configKey, mappedIndex);
      ...
      for (var actualHeader in actualHeaderIndices) {
        ...
        if (normalizedActualHeader === normalizedConfigName) {
          ...
          console.log('mapConfigToActualHeaders: Case-insensitive match found for %s: "%s" -> index %s', configKey, actualHeader, mappedIndex);
          break;
        }
      }
      ...
      if (mappedIndex === undefined) {
        for (var actualHeader in actualHeaderIndices) {
          ...
          if (normalizedActualHeader.includes(normalizedConfigName) || normalizedConfigName.includes(normalizedActualHeader)) {
            mappedIndex = actualHeaderIndices[actualHeader];
            console.log('mapConfigToActualHeaders: Partial match found for %s: "%s" -> index %s', configKey, actualHeader, mappedIndex);
            break;
          }
        }
      }
    ...
    if (mappedIndex === undefined && configKey === 'opinionHeader') {
      ...
      if (questionHeaders.length > 0) {
        ...
        console.log('mapConfigToActualHeaders: Auto-detected main question header for %s: "%s" -> index %s', configKey, longestHeader.header, mappedIndex);
      }
    }
    ...
    if (mappedIndex === undefined && configKey === 'reasonHeader') {
      ...
      console.log('mapConfigToActualHeaders: Searching for reason header with keywords: %s', JSON.stringify(reasonKeywords));
      ...
      for (var header in actualHeaderIndices) {
        ...
        for (var k = 0; k < reasonKeywords.length; k++) {
          if (normalizedHeader.includes(reasonKeywords[k]) || reasonKeywords[k].includes(normalizedHeader)) {
            ...
            console.log('mapConfigToActualHeaders: Auto-detected reason header for %s: "%s" -> index %s (keyword: %s)', configKey, header, mappedIndex, reasonKeywords[k]);
            break;
          }
        }
      ...
      if (mappedIndex === undefined) {
        for (var header in actualHeaderIndices) {
          ...
          if (normalizedHeader.indexOf('理由') !== -1 || normalizedHeader.indexOf('reason') !== -1) {
            mappedIndex = actualHeaderIndices[header];
            console.log('mapConfigToActualHeaders: Found reason header by partial match for %s: "%s" -> index %s', configKey, header, mappedIndex);
            break;
          }
        }
      }
    ...
    if (mappedIndex === undefined) {
      console.log('mapConfigToActualHeaders: WARNING - No match found for %s = "%s"', configKey, configHeaderName);
    }
  }
...
function updateIsActiveStatus(requestUserId, isActive) {
  ...
  try {
    ...
    // 現在のユーザー情報を取得
    var userInfo = DB.findUserByEmail(activeUserEmail);
    if (!userInfo) {
      return {
        status: 'error',
