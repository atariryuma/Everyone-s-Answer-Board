...
class UnifiedErrorHandler {
   ...
   * @param {string} [severity=UNIFIED_CONSTANTS.ERROR.SEVERITY.MEDIUM] エラーの重要度
   * @param {string} [category=UNIFIED_CONSTANTS.ERROR.CATEGORIES.SYSTEM] エラーカテゴリ
  ...
  logError(
    error,
    context,
    severity = UNIFIED_CONSTANTS.ERROR.SEVERITY.MEDIUM,
    category = UNIFIED_CONSTANTS.ERROR.CATEGORIES.SYSTEM,
    metadata = {}
  ) {
    this.errorCount++;
    ...
    // 重要度に応じた出力方法
    switch (severity) {
      case UNIFIED_CONSTANTS.ERROR.SEVERITY.CRITICAL:
        ULog.error(`🚨 CRITICAL ERROR [${context}]:`, JSON.stringify(errorInfo, null, 2));
        break;
      case UNIFIED_CONSTANTS.ERROR.SEVERITY.HIGH:
        ULog.error(`❌ HIGH SEVERITY [${context}]:`, JSON.stringify(errorInfo, null, 2));
        break;
      case UNIFIED_CONSTANTS.ERROR.SEVERITY.MEDIUM:
        ULog.warn(`⚠️ MEDIUM SEVERITY [${context}]:`, errorInfo.message, errorInfo.metadata);
        break;
      case UNIFIED_CONSTANTS.ERROR.SEVERITY.LOW:
        ULog.info(`ℹ️ LOW SEVERITY [${context}]:`, errorInfo.message);
        break;
    }
  ...
  logDatabaseError(error, operation, operationDetails = {}) {
    try {
      ...
      ULog.error(`⚠️ LOGGING ERROR [database.${operation}]:`, {
      ...
      return {
        ...
        severity: UNIFIED_CONSTANTS.ERROR.SEVERITY.MEDIUM,
        category: UNIFIED_CONSTANTS.ERROR.CATEGORIES.DATABASE,
  ...
  logValidationError(field, value, rule, message) {
    ...
    return this.logError(
      message,
      `validation.${field}`,
      UNIFIED_CONSTANTS.ERROR.SEVERITY.LOW,
...
if (typeof warnLog === 'undefined') {
  function warnLog(message, ...args) {
    ULog.warn('[WARN]', message, ...args);
  }
}

// errorLog統合: 上記のlogError関数を使用。重複定義削除

if (typeof infoLog === 'undefined') {
  function infoLog(message, ...args) {
    ULog.info('[INFO]', message, ...args);
  }
}
...
function getSetupStep(userInfo, configJson) {
  ULog.debug('🔍 getSetupStep: UI状態ベースのステップ判定開始', {
  ...
  if (
    ...
    configJson.unpublishReason === 'manual_stop'
  ) {
    ULog.debug('🔧 ステップ1判定: 手動停止による完全リセット', {
  ...
  if (!configJson || typeof configJson !== 'object') {
    ULog.debug('🔧 ステップ2判定: config未設定', { configJson: typeof configJson });
    return 2;
  }

  // Step 3: 公開中の判定（最優先）
  if (configJson.appPublished === true) {
    ULog.debug('🔧 ステップ3判定: 現在公開中', { appPublished: configJson.appPublished });
    return 3;
  }
  ...
  const hasRequiredSettings =
    ...
    configJson.opinionHeader.trim() !== '' && // 意見列が設定済み
    configJson.activeSheetName &&
    configJson.activeSheetName.trim() !== ''; // アクティブシートが選択済み

  ULog.debug('🔍 設定完成度チェック', {
  ...
  if (hasRequiredSettings) {
    ULog.debug('🔧 ステップ3判定: 設定完了（公開可能状態）', {
      reason: '必要設定が完了済み',
    });
    return 3;
  }
...
function clearActiveSheet(requestUserId) {
  ...
  return executeWithStandardizedLock('WRITE_OPERATION', 'clearActiveSheet', () => {
    ULog.debug('clearActiveSheet開始: userId=' + requestUserId);
    ...
    ULog.info('clearActiveSheet完了: 公開を停止しました');
...
function determineSetupStepUnified(userInfo, configJson, options = {}) {
  ...
  // Step 1: データソース未設定
  if (!userInfo || !userInfo.spreadsheetId || userInfo.spreadsheetId.trim() === '') {
    if (debugMode) ULog.debug('🔧 setupStep統一判定: Step 1 - データソース未設定');
    return 1;
  }

  // configJsonが存在しない場合は必ずStep 2
  if (!configJson || typeof configJson !== 'object') {
    if (debugMode) ULog.debug('🔧 setupStep統一判定: Step 2 - configJson未設定');
    return 2;
  }
  ...
  if (isStep2) {
    if (debugMode) {
      ULog.debug('🔧 setupStep統一判定: Step 2 - セットアップ未完了', {
        setupStatus,
        formCreated,
        hasFormUrl,
      });
  ...
  if (setupStatus === 'completed' && formCreated && hasFormUrl) {
    if (debugMode) ULog.debug('🔧 setupStep統一判定: Step 3 - セットアップ完了');
    return 3;
  }

  // フォールバック: 不明な状態はStep 2として扱う
  if (debugMode) {
    ULog.debug('🔧 setupStep統一判定: フォールバック - Step 2', {
      setupStatus,
      formCreated,
      hasFormUrl,
    });
...
function validateConfigJson(config) {
  try {
    // 統一Validationシステムを使用（フォールバック付き）
    if (typeof UnifiedValidation !== 'undefined') {
      const result = UnifiedValidation.validate('configuration', 'basic', { config });
      return convertToLegacyFormat('validateConfigJson', result);
    }
    ...
    for (const [field, expectedType] of Object.entries(requiredFields)) {
      if (config[field] === undefined) {
        ...
        errors.push(
          `フィールド '${field}' の型が不正です。期待値: ${expectedType}, 実際の値: ${typeof config[field]}`
        );
      }
    }
    ...
    ULog('ERROR', 'validateConfigJson', 'Config JSON validation failed', { 
      error: error.message,
...
function parseAndValidateConfigJson(configJsonString) {
  ...
  try {
    ...
    ULog.warn('configJson解析エラー:', parseError.message);
    config = {};
  }
  ...
  if (mergedConfig.publishedSheetName === 'true') {
    ...
    ULog.warn('publishedSheetNameの不正値を修正しました');
  }

  const validation = validateConfigJson(mergedConfig);
  if (!validation.isValid) {
    ULog.warn('configJson検証警告:', validation.errors);
  }
...
function performAutoHealing(userInfo, configJson, userId) {
  ...
  try {
    ...
    if (
      healedConfig.publishedSheetName &&
    ...
    if (updated) {
      ...
      if (!validation.isValid) {
        logError(
          `Auto-healing後の状態が無効: ${validation.errors}`,
          'autoHealConfig',
      ...
      if (validation.warnings.length > 0) {
        ULog.warn('⚠️ Auto-healing後の警告:', validation.warnings);
      }
    }
    ...
    if (updated && userId) {
      try {
        ...
        ULog.debug('📋 Auto-healing実行:', changes.join(', '));
      } catch (updateError) {
...
function validateConfigJsonState(configJson, userInfo) {
  ...
  const formCreated = !!configJson.formCreated;
  const hasFormUrl = !!(configJson.formUrl && configJson.formUrl.trim());
  const hasPublishedSheet = !!(
    configJson.publishedSheetName && configJson.publishedSheetName.trim()
  );
...
function safeStateTransition(currentConfig, newValues, userInfo) {
  ...
  if (!validation.isValid) {
    return createErrorResponse(
      ...
      '状態遷移の検証に失敗しました',
      { configJson: currentConfig, errors: validation.errors }
    );
  }

  // 遷移実行
  ULog.info('✅ 状態遷移実行:', Object.keys(newValues).join(', '));
  if (validation.warnings.length > 0) {
    ULog.warn('⚠️ 状態遷移警告:', validation.warnings.join(', '));
  }

  return createSuccessResponse(
    { configJson: transitionConfig, errors: [] },
    '状態遷移が正常に完了しました'
  );
}
// =================================================================
...
function validateHeaderIntegrity(userId) {
  try {
    ULog.debug('🔍 Starting header integrity validation for userId:', userId);

    const userInfo = getOrFetchUserInfo(userId, 'userId');
    if (!userInfo || !userInfo.spreadsheetId) {
      return createErrorResponse(
        'User spreadsheet not found',
        'ユーザーのスプレッドシートが見つかりません',
        { timestamp: new Date().toISOString() }
      );
    }
    ...
    let validationResults = createSuccessResponse(headerValidationData);

    // 理由列の必須チェック
    if (indices[COLUMN_HEADERS.REASON] === undefined) {
      headerValidationData.issues.push('Reason column (理由) not found in headers');
    }

    // 回答列の必須チェック
    if (indices[COLUMN_HEADERS.OPINION] === undefined) {
      headerValidationData.issues.push('Opinion column (回答) not found in headers');
    }

    // バリデーション結果の更新
    if (headerValidationData.issues.length > 0) {
      validationResults = createErrorResponse('Header integrity validation failed', headerValidationData.issues.join(', '), headerValidationData);
    }

    // ログ出力
    if (validationResults.success) {
      ULog.info('✅ Header integrity validation passed');
    } else {
      ULog.warn('⚠️ Header integrity validation failed:', headerValidationData.issues);
    }
...
function getOpinionHeaderSafely(userId, sheetName) {
  try {
    ...
    ULog.debug('getOpinionHeaderSafely:', {
      userId: userId,
      sheetName: sheetName,
...
function registerNewUser(adminEmail) {
  ...
  if (existingUser) {
    ...
    ULog.info('✅ 既存ユーザーの最終アクセス時刻を更新しました（設定は保護）: ' + adminEmail);
  ...
  try {
    createUser(userData);
    ULog.info('✅ データベースに新規ユーザーを登録しました: ' + adminEmail);
    ...
    // 原子的な作成→検証フロー（最大3秒待機）
    ULog.debug('registerNewUser: データベース同期検証を開始...');
        ...
        for (const method of verificationMethods) {
          try {
            ...
            if (createdUser && createdUser.userId === userId) {
              ...
              ULog.debug('✅ registerNewUser: データベース同期確認成功', {
                stage: stage.method,
            ...
            ULog.debug('registerNewUser: 検証方法でエラー:', methodError.message);
          }
        }

        if (verificationSuccess) break;
      } catch (stageError) {
        ULog.warn('registerNewUser: 検証ステージでエラー:', {
          stage: stage.method,
    ...
    if (!verificationSuccess || !createdUser) {
      ...
      ULog.error('[ERROR]', '❌ registerNewUser: データベース同期検証失敗', {
        userId,
        email: adminEmail,
        attempts: attemptCount,
      ...
      throw new Error(
        `ユーザーデータの作成を確認できませんでした。${attemptCount}回の検証を実行しましたが、データベースの同期が完了していません。しばらく待ってから再試行してください。`
      );
    }
    ...
    try {
      ...
      ULog.debug('✅ registerNewUser: 検証成功後キャッシュ設定完了');
    } catch (cacheError) {
      ULog.warn('registerNewUser: キャッシュ設定でエラー:', cacheError.message);
    ...
    if (
      messageText.indexOf('既に登録されています') !== -1 ||
      messageText.toLowerCase().indexOf('duplicate') !== -1
    ) {
      ULog.warn('registerNewUser: メール重複検出、既存ユーザー検索中...', adminEmail);
      try {
        ...
        ULog.warn('registerNewUser: 既存ユーザー検索でエラー:', lookupError.message);
      }
    }

    ULog.error('[ERROR]', '❌ registerNewUser: ユーザー登録失敗:', {
      error: e.message,
      userId,
      email: adminEmail,
    });
...
function isPublishedBoard(userInfo) {
  ...
  try {
    ...
    ULog.debug('Config JSON parse error in isPublishedBoard:', e.message);
    return false;
  }
}
...
function openSpreadsheetWithServiceAccount(spreadsheetId) {
  try {
    ...
    ULog.debug('サービスアカウントでのスプレッドシートアクセスエラー:', error.message);
    // フォールバック: 通常のアクセス
    return SpreadsheetApp.openById(spreadsheetId);
  }
}
...
function addReaction(requestUserId, rowIndex, reactionKey, sheetName) {
  ...
  try {
    ...
    logError(
      ...
      UNIFIED_CONSTANTS.ERROR.SEVERITY.MEDIUM,
      UNIFIED_CONSTANTS.ERROR.CATEGORIES.SYSTEM,
      { userId: requestUserId, rowIndex, reaction: reactionKey }
    );
    return createErrorResponse(e.message, 'リアクション追加でエラーが発生しました', null);
  }
...
function addReactionBatch(requestUserId, batchOperations) {
  ...
  try {
    ...
    ULog.debug('🔄 バッチリアクション処理開始:', batchOperations.length + '件');
    ...
    for (var i = 0; i < batchOperations.length; i++) {
      ...
      try {
        ...
        if (!operation.rowIndex || !operation.reaction) {
          ULog.warn('無効な操作をスキップ:', operation);
          continue;
        }
        ...
        if (result && result.success) {
          ...
          batchResults.push(createSuccessResponse({
          ...
          ULog.warn('リアクション処理失敗:', operation, result.message);
          batchResults.push(createErrorResponse(
            result.message || 'リアクション処理失敗',
            null,
            {
              rowIndex: operation.rowIndex,
              reaction: operation.reaction,
    ...
    processedRows.forEach(function (rowIndex) {
      try {
        ...
        ULog.warn('最終状態取得エラー:', rowIndex, error.message);
      }
    });

    ULog.info('✅ バッチリアクション処理完了:', {
      total: batchOperations.length,
      processed: processedRows.size,
      success: batchResults.filter((r) => r.success === true).length,
    });

    return createSuccessResponse({
      data: finalResults,
      processedCount: batchOperations.length,
      successCount: batchResults.filter((r) => r.success === true).length,
      timestamp: new Date().toISOString(),
...
function getCurrentSheetName(spreadsheetId) {
  try {
    ...
    ULog.warn('シート名取得エラー:', error.message);
...
function verifyUserAccess(requestUserId) {
  ...
  const activeUserEmail = getCurrentUserEmail();
  ULog.debug(`verifyUserAccess start: userId=${requestUserId}, email=${activeUserEmail}`);
  if (!activeUserEmail) {
    throw new Error('認証エラー: アクティブユーザーの情報を取得できませんでした');
  }
  ...
  if (requestUserId == null) {
    ULog.debug(`✅ 新規ユーザーセッション検証成功: ${activeUserEmail}`);
    return;
  }
  ...
  if (activeUserEmail !== requestedUserInfo.adminEmail) {
    ...
    if (config.appPublished === true) {
      ULog.debug(`✅ 公開ボード閲覧許可: ${activeUserEmail} -> ${requestUserId}`);
      return;
    }
    throw new Error(
...
function getPublishedSheetData(requestUserId, classFilter, sortOrder, adminMode, bypassCache) {
  // 統一Sheet管理システムを使用（互換性維持）
  return getPublishedSheetDataUnified(requestUserId, classFilter, sortOrder, adminMode, bypassCache);
}
...
function executeGetPublishedSheetData(requestUserId, classFilter, sortOrder, adminMode) {
  try {
    ...
    ULog.debug('getPublishedSheetData: userInfo=%s', JSON.stringify(userInfo));

    var configJson = JSON.parse(userInfo.configJson || '{}');
    ULog.debug('getPublishedSheetData: configJson=%s', JSON.stringify(configJson));
    ...
    ULog.debug('getPublishedSheetData: sheetConfig=%s', JSON.stringify(sheetConfig));
    ...
    try {
      ULog.debug('🔍 診断: スプレッドシート詳細情報');
      ULog.debug('  publishedSpreadsheetId:', publishedSpreadsheetId);
      ULog.debug('  publishedSheetName:', publishedSheetName);
      ULog.debug('  データ取得結果:', {
      ...
      if (sheetData.totalCount === 0) {
        ULog.debug('⚠️ 診断: データが0件です。原因を調査します...');
        ...
        if (sheet) {
          ...
          if (lastRow <= 1) {
            ULog.debug('⚠️ 診断結果: スプレッドシートにデータ行がありません（ヘッダーのみ）');
          }
        } else {
          ULog.debug('❌ 診断結果: 指定されたシート名が見つかりません');
        }
      }
    } catch (diagnosisError) {
      ULog.warn('診断処理でエラー:', diagnosisError.message);
    }
    ...
    ULog.debug('getPublishedSheetData: Available headerIndices=%s', JSON.stringify(headerIndices));
    ...
    ULog.debug('getPublishedSheetData: Mapped indices=%s', JSON.stringify(mappedIndices));

    var formattedData = formatSheetDataForFrontend(
      sheetData.data,
      mappedIndices,
      headerIndices,
      adminMode,
    ...
    ULog.debug('getPublishedSheetData: formattedData length=%s', formattedData.length);
    ULog.debug('getPublishedSheetData: formattedData content=%s', JSON.stringify(formattedData));
    ...
    if (mappedIndices.opinionHeader !== undefined) {
      for (var actualHeader in headerIndices) {
        if (headerIndices[actualHeader] === mappedIndices.opinionHeader) {
          ...
          ULog.debug('getPublishedSheetData: Using actual header as title: "%s"', headerTitle);
          break;
        }
      }
    }

    var finalDisplayMode =
    ...
    ULog.debug('getPublishedSheetData: Returning result=%s', JSON.stringify(result));
    return result;
  } catch (e) {
    logError(
      e,
      ...
      UNIFIED_CONSTANTS.ERROR.SEVERITY.MEDIUM,
      UNIFIED_CONSTANTS.ERROR.CATEGORIES.DATABASE,
      { userId, publishedSpreadsheetId, publishedSheetName }
    );
    return createErrorResponse(e.message, 'データの取得に失敗しました: ' + e.message, {
      data: [],
      rows: [],
    });
  }
...
function getIncrementalSheetData(requestUserId, classFilter, sortOrder, adminMode, sinceRowCount) {
  // 統一Sheet管理システムを使用（互換性維持）
  return getIncrementalSheetDataUnified(requestUserId, classFilter, sortOrder, adminMode, sinceRowCount);
}

/**
 * 従来の増分データ取得実装（レガシー用に保持）
 * @deprecated 統一システムを使用してください
 */
function getIncrementalSheetDataLegacy(requestUserId, classFilter, sortOrder, adminMode, sinceRowCount) {
  ...
  try {
    ULog.debug('🔄 増分データ取得開始: sinceRowCount=%s', sinceRowCount);
    ...
    ULog.debug('DEBUG: Spreadsheet object obtained: %s', ss ? ss.getName() : 'null');

    var sheet = ss.getSheetByName(publishedSheetName);
    ULog.debug('DEBUG: Sheet object obtained: %s', sheet ? sheet.getName() : 'null');
    ...
    ULog.debug('📥 スプレッドシートから直接取得した新しいデータ:', rawNewData.length, '件');
    ...
    var formattedNewData = formatSheetDataForFrontend(
      processedData,
      mappedIndices,
      headerIndices,
      adminMode,
    ...
    ULog.info('✅ 増分データ取得完了: %s件の新しいデータを返します', formattedNewData.length);
    ...
    return createErrorResponse('増分データの取得に失敗しました: ' + e.message);
  }
}
...
function getAvailableSheets(requestUserId) {
  ...
  try {
    ...
    if (!currentUserId) {
      ULog.warn('getAvailableSheets: No current user ID set');
      return [];
    }
    ...
    if (!sheets || sheets.length === 0) {
      ULog.warn('getAvailableSheets: No sheets found for user:', currentUserId);
      return [];
    }
 ...
 */
// 古いgetSheetsList実装は削除（4905行目の詳細実装を使用）
...
function refreshBoardData(requestUserId) {
  ...
  try {
    ...
    return createErrorResponse(e.message, 'ボードデータの再読み込みに失敗しました: ' + e.message, null);
  }
}
...
function formatSheetDataForFrontend(
  ...
  return rawData.map(function (row, index) {
    ...
    ULog.debug(
      'formatSheetDataForFrontend: Content extraction for row %s - opinion="%s", reason="%s"',
      index,
      opinionValue.substring(0, 50),
      finalReasonValue.substring(0, 50)
    );
...
function getAppConfig(requestUserId) {
  ...
  try {
    ...
    try {
      ...
      ULog.warn('回答数の取得に失敗: ' + countError.message);
    }
    ...
    try {
      const activeSheet =
        configJson && typeof configJson === 'object' ? configJson.publishedSheetName || '' : '';
      if (activeSheet) {
      ...
      ULog.debug('getAppConfig: activeFormUrl 解決スキップ:', e.message);
    }
    ...
    return createErrorResponse(e.message, '設定の取得に失敗しました: ' + e.message);
  }
}
...
function saveSheetConfig(userId, spreadsheetId, sheetName, config, options = {}) {
  try {
    ...
    ULog.info('✅ シート設定を保存しました: %s', sheetKey);
    return createSuccessResponse(null, 'シート設定を保存しました。');
  } catch (e) {
    logDatabaseError(e, 'saveSheetSettings', { userId });
    return createErrorResponse(e.message, 'シート設定の保存に失敗しました: ' + e.message, null);
  }
}
...
function switchToSheet(userId, spreadsheetId, sheetName, options = {}) {
  try {
    ...
    updateUser(currentUserId, { configJson: JSON.stringify(configJson) });
    ULog.info('✅ 表示シートを切り替えました: %s - %s', spreadsheetId, sheetName);
    return createSuccessResponse(null, '表示シートを切り替えました。');
  } catch (e) {
    logError(
      e,
      'switchSheet',
    ...
    return createErrorResponse(e.message, '表示シートの切り替えに失敗しました: ' + e.message, null);
  }
}
...
function getResponsesData(userId, sheetName) {
  ...
  try {
    ...
    if (values.length === 0) {
      return createSuccessResponse({ data: [], headers: [] }, 'データなし');
    }
    ...
    return createErrorResponse(e.message, '回答データの取得に失敗しました: ' + e.message, null);
  }
}
...
function getActiveFormInfo(requestUserId) {
  ...
  try {
    ...
    // デバッグログを追加
    ULog.debug('🔍 getActiveFormInfo: configJson分析', {
      userId: currentUserId,
    ...
    return createErrorResponse(e.message, 'フォーム情報の取得に失敗しました: ' + e.message, null);
  }
}
...
function checkAdmin(requestUserId) {
  ...
  try {
    ...
    ULog.error('[ERROR]', 'checkAdmin エラー: ' + e.message);
    return false;
  }
}
...
function getDataCount(requestUserId, classFilter, sortOrder, adminMode) {
  ...
  try {
    ...
    if (!configJson.publishedSpreadsheetId || !configJson.publishedSheetName) {
      return createErrorResponse('スプレッドシート設定なし', 'スプレッドシート設定なし', {
        count: 0,
    ...
    ULog.error('[ERROR]', 'getDataCount エラー: ' + e.message);
    return createErrorResponse(e.message, e.message, {
      count: 0,
      lastUpdate: new Date().toISOString(),
    });
  }
  ...
  try {
    ...
    if (configJson.editFormUrl) {
      try {
        ...
        return createSuccessResponse(null, 'フォーム設定が更新されました');
      } catch (formError) {
        ULog.error('[ERROR]', 'フォーム更新エラー: ' + formError.message);
        return createErrorResponse(formError.message, 'フォームの更新に失敗しました: ' + formError.message, null);
      }
    } else {
      return createErrorResponse('FORM_NOT_FOUND', 'フォームが見つかりません', null);
    }
  } catch (e) {
    ULog.error('[ERROR]', 'updateFormSettings エラー: ' + e.message);
    return createErrorResponse(e.message, 'フォーム設定の更新に失敗しました: ' + e.message, null);
  }
}
...
function saveSystemConfig(requestUserId, config) {
  ...
  try {
    ...
    return createSuccessResponse(null, 'システム設定が保存されました');
  } catch (e) {
    ULog.error('[ERROR]', 'saveSystemConfig エラー: ' + e.message);
    return createErrorResponse(e.message, 'システム設定の保存に失敗しました: ' + e.message, null);
  }
}
...
function toggleHighlight(requestUserId, rowIndex, sheetName) {
  ...
  try {
    ...
    ULog.error('[ERROR]', 'toggleHighlight エラー: ' + e.message);
    return createErrorResponse(e.message, 'ハイライト切り替えでエラーが発生しました', null);
  }
}
    ...
    try {
      ...
      try {
        ...
        if (!isFormAlreadyInFolder) {
          ULog.debug('📝 フォームファイルを移動中: %s → %s', formFile.getId(), folder.getName());

          // 推奨メソッドmoveTo()を使用してファイル移動
          formFile.moveTo(folder);
          moveResults.form = true;
          ULog.info('✅ フォームファイル移動完了: %s (ID: %s)', folder.getName(), formFile.getId());
        } else {
          ULog.debug('ℹ️ フォームファイルは既にフォルダに存在します');
          moveResults.form = true;
        }
      } catch (formMoveError) {
        moveErrors.push('フォームファイル移動エラー: ' + formMoveError.message);
        ULog.error('[ERROR]', '❌ フォームファイルの移動に失敗:', formMoveError.message);
        ULog.error(
          '[ERROR]',
          'フォームID: %s, フォルダID: %s',
          formAndSsInfo.formId,
          folder ? folder.getId() : 'なし'
        );
      }
      ...
      try {
        ...
        if (!isSsAlreadyInFolder) {
          ...
          ULog.info(
            '✅ スプレッドシートファイル移動完了: %s (ID: %s)',
            folder.getName(),
            ssFile.getId()
          );
        } else {
          ULog.debug('ℹ️ スプレッドシートファイルは既にフォルダに存在します');
          moveResults.spreadsheet = true;
        }
      } catch (ssMoveError) {
        moveErrors.push('スプレッドシートファイル移動エラー: ' + ssMoveError.message);
        ULog.error('[ERROR]', '❌ スプレッドシートファイルの移動に失敗:', ssMoveError.message);
        ULog.error(
          '[ERROR]',
          'スプレッドシートID: %s, フォルダID: %s',
      ...
      if (moveResults.form && moveResults.spreadsheet) {
        ULog.info('✅ 全ファイルのフォルダ移動が完了: ' + folder.getName());
      } else {
        ULog.warn('⚠️ 一部のファイル移動に失敗しましたが、処理を継続します');
        ULog.debug(
          '移動結果: フォーム=%s, スプレッドシート=%s',
          moveResults.form,
          moveResults.spreadsheet
        );
        if (moveErrors.length > 0) {
          ULog.debug('移動エラー詳細: %s', moveErrors.join('; '));
        }
      }
    } catch (generalError) {
      ULog.error('[ERROR]', '❌ ファイル移動処理で予期しないエラー:', generalError.message);
      // ファイル移動失敗は致命的ではないため、処理は継続
      ULog.debug('ファイルはマイドライブに残りますが、システムは正常に動作します');
    }
  }
...
function updateQuickStartDatabase(setupContext, createdFiles) {
  ...
  ULog.debug('💾 ステップ3: データベース更新中...');
  ULog.debug('📊 新規作成されたファイル情報:');
  ULog.debug('  📝 フォームID:', formAndSsInfo.formId);
  ULog.debug('  📊 スプレッドシートID:', formAndSsInfo.spreadsheetId);
  ULog.debug('  📄 シート名:', formAndSsInfo.sheetName);
  ...
  ULog.debug('📝 クイックスタート用質問文設定:');
  ULog.debug('  💭 メイン質問:', quickStartSheetConfig.guessedConfig.opinionHeader);
  ULog.debug('  💡 理由質問:', quickStartSheetConfig.guessedConfig.reasonHeader);
  ...
  if (typeof safeSheetName !== 'string') {
    ULog.error(
      '[ERROR]',
      '❌ quickStartSetup: formAndSsInfo.sheetNameが文字列ではありません:',
      typeof safeSheetName,
      safeSheetName
    );
    safeSheetName = String(safeSheetName); // 強制的に文字列化
  }
  if (safeSheetName === 'true' || safeSheetName === 'false') {
    ULog.error('[ERROR]', '❌ quickStartSetup: 無効なシート名が検出されました:', safeSheetName);
  ...
  ULog.debug('💾 ユーザーデータベースを新しいセットアップで更新中...');
  var updateData = {
  ...
  ULog.debug('📋 データベース更新内容:');
  ULog.debug('  📊 新スプレッドシートID:', updateData.spreadsheetId);
  ULog.debug('  🔗 新スプレッドシートURL:', updateData.spreadsheetUrl);

  updateUser(requestUserId, updateData);

  ULog.info('✅ ユーザーデータベース更新完了!');

  // 重要: 新しいセットアップ完了後に包括的キャッシュ同期を実行（二重保証）
  ULog.debug('🗑️ 新しいセットアップ確実反映のための包括的キャッシュ同期中...');
  ...
  ULog.debug('🔍 データベース更新の最終検証中...');
  ULog.debug('🎯 期待するスプレッドシートID:', formAndSsInfo.spreadsheetId);
  ...
  ULog.debug(
    '  取得したスプレッドシートID:',
    verificationUserInfo ? verificationUserInfo.spreadsheetId : 'null'
  );
  ULog.debug(
    '  期待値との一致:',
    verificationUserInfo && verificationUserInfo.spreadsheetId === formAndSsInfo.spreadsheetId
  );

  if (verificationUserInfo && verificationUserInfo.spreadsheetId === formAndSsInfo.spreadsheetId) {
    ULog.info('✅ データベース更新検証成功: 新しいスプレッドシートID確認');
  } else {
    ULog.warn('⚠️ データベース更新検証で不一致が検出されましたが、処理を続行します');
    ULog.debug('📝 注意: この不一致は一時的なキャッシュの問題の可能性があります');

    // 検証失敗でもQuick Startを続行する（フォーム作成は成功している）
    ULog.debug('🚀 フォーム作成は成功しているため、Quick Startを続行します');
  }
...
function performAutoPublish(requestUserId, sheetName) {
  try {
    ULog.debug('🌐 QuickStart自動公開開始', { requestUserId, sheetName });
    ...
    if (publishResult && publishResult.success !== false) {
      ...
      throw new Error(
        'setActiveSheetが失敗しました: ' + (publishResult?.message || 'unknown error')
      );
    }
  } catch (error) {
    ULog.error('[ERROR]', '❌ QuickStart自動公開エラー', {
      requestUserId,
      sheetName,
      error: error.message,
      stack: error.stack,
    ...
    return createErrorResponse(error.message, '自動公開に失敗しましたが、手動で公開できます', {
      published: false,
      sheetName: sheetName,
...
function generateQuickStartResponse(setupContext, createdFiles, updatedConfig, publishResult) {
  ...
  // 最終検証：新規作成されたファイルの確認
  ULog.debug('🔍 最終検証 - 作成されたファイル:');
  ULog.debug('  📝 フォームID:', formAndSsInfo.formId);
  ULog.debug('  📊 スプレッドシートID:', formAndSsInfo.spreadsheetId);
  ULog.debug('  📄 シート名:', formAndSsInfo.sheetName);
  ULog.debug('  🔗 フォームURL:', formAndSsInfo.formUrl);
  ULog.debug('  🔗 スプレッドシートURL:', formAndSsInfo.spreadsheetUrl);
  ...
  ULog.info('✅ クイックスタートセットアップ完了: ' + requestUserId);
  ...
  var response = createSuccessResponse(responseData, 'クイックスタートが完了しました！' + publishMessage);

  ULog.debug('📤 QuickStart最終レスポンス生成完了:', response);
  return response;
}
...
function initializeQuickStartContext(requestUserId) {
  ULog.debug('🚀 クイックスタートセットアップ開始: ' + requestUserId);
  ...
  if (configJson.formCreated || userInfo.spreadsheetId) {
    ULog.debug('⚠️ 既存のセットアップが検出されました。新しいセットアップで完全に上書きします。');
    ULog.debug('🔄 既存スプレッドシートID:', userInfo.spreadsheetId);
    ...
    ULog.debug('🗑️ 既存スプレッドシートIDをクリアして新規作成を強制します');
...
function quickStartSetup(requestUserId) {
  verifyUserAccess(requestUserId);
  try {
    ULog.debug('🚀 QuickStart: セットアップ開始', { requestUserId });

    // ステップ0: セットアップコンテキストを初期化
    ULog.debug('📋 QuickStart: ステップ0 - コンテキスト初期化中...');
    var setupContext = initializeQuickStartContext(requestUserId);
    var configJson = setupContext.configJson;
    var userEmail = setupContext.userEmail;
    var userInfo = setupContext.userInfo;
    ULog.debug('✅ QuickStart: ステップ0完了 - コンテキスト初期化成功');

    // ステップ1-2: ファイル作成とフォルダ管理を実行
    ULog.debug('📁 QuickStart: ステップ1-2 - ファイル・フォルダ作成中...');
    var createdFiles = createQuickStartFiles(setupContext);
    var formAndSsInfo = createdFiles.formAndSsInfo;
    var folder = createdFiles.folder;
    ULog.debug('✅ QuickStart: ステップ1-2完了 - ファイル作成成功', {
      formId: formAndSsInfo.formId,
      spreadsheetId: formAndSsInfo.spreadsheetId,
      sheetName: formAndSsInfo.sheetName,
    });
    ...
    ULog.debug('💾 QuickStart: ステップ3 - データベース更新中...');
    var updatedConfig = updateQuickStartDatabase(setupContext, createdFiles);
    ULog.debug('✅ QuickStart: ステップ3完了 - データベース更新成功');

    // ステップ4: 自動公開処理（重要な新機能）
    ULog.debug('🌐 QuickStart: ステップ4 - 自動公開処理中...');
    var publishResult = performAutoPublish(requestUserId, formAndSsInfo.sheetName);
    ULog.debug('✅ QuickStart: ステップ4完了 - 自動公開成功', publishResult);

    // ステップ5: キャッシュクリアと最終化
    ULog.debug('🔄 QuickStart: ステップ5 - キャッシュクリア中...');
    clearExecutionUserInfoCache();
    invalidateUserCache(requestUserId, userEmail, createdFiles.formAndSsInfo.spreadsheetId, true);
    ULog.debug('✅ QuickStart: ステップ5完了 - キャッシュクリア成功');

    // ステップ6: 最終レスポンス生成
    ULog.debug('📤 QuickStart: ステップ6 - レスポンス生成中...');
    var finalResponse = generateQuickStartResponse(
      setupContext,
      createdFiles,
      updatedConfig,
      publishResult
    );
    ULog.debug('🎉 QuickStart: 全工程完了！', finalResponse);

    return finalResponse;
  } catch (e) {
    ULog.error('[ERROR]', '❌ quickStartSetup エラー: ' + e.message);
    ...
    try {
      ...
      ULog.error('[ERROR]', 'エラー状態の更新に失敗: ' + updateError.message);
    }

    return createErrorResponse(e.message, 'クイックスタートセットアップに失敗しました: ' + e.message, {
      webAppUrl: '',
      adminUrl: '',
      viewUrl: '',
      setupUrl: '',
...
function createUserFolder(userEmail) {
  try {
    ...
    ULog.error('[ERROR]', 'createUserFolder エラー: ' + e.message);
  ...
  try {
    ...
    try {
      if (
        ...
        ULog.debug('ハイライト更新後のキャッシュ無効化完了: ' + spreadsheetId);
      }
    } catch (cacheError) {
      ULog.warn('ハイライト後のキャッシュ無効化エラー:', cacheError.message);
    }

    return createSuccessResponse({
      highlighted: !isHighlighted,
    }, isHighlighted ? 'ハイライトを解除しました' : 'ハイライトしました');
  } catch (e) {
    ULog.error('[ERROR]', 'ハイライト処理エラー: ' + e.message);
    return createErrorResponse(e.message, e.message, null);
  }
}
...
function getHeaderIndices(spreadsheetId, sheetName) {
  ...
  if (!indices || indices[COLUMN_HEADERS.REASON] === undefined) {
    ULog.debug('getHeaderIndices: Reason header missing, refreshing cache for key=%s', cacheKey);
...
function getSheetColumns(userId, sheetId) {
  ...
  try {
    ...
    ULog.info('✅ getSheetColumns: Found %s columns for sheetId %s', columns.length, sheetId);
    return columns;
  } catch (e) {
    ULog.error('[ERROR]', 'getSheetColumns エラー: ' + e.message);
    ULog.error('[ERROR]', 'Error details:', e.stack);
...
function extractFormIdFromUrl(url) {
  ...
  try {
    ...
    ULog.warn('フォームID抽出エラー: ' + e.message);
    return '';
  }
}
...
function processReaction(spreadsheetId, sheetName, rowIndex, reactionKey, reactingUserEmail) {
  try {
    ...
    return executeWithStandardizedLock('WRITE_OPERATION', 'processReaction', () => {
      ...
      REACTION_KEYS.forEach(function (key) {
        ...
        if (key === reactionKey) {
          ...
          if (userIndex >= 0) {
            currentReactions.splice(userIndex, 1);
            ULog.debug('他のリアクションから削除: ' + reactingUserEmail + ' from ' + key);
          }
        }
      ...
      try {
        if (
          ...
          cacheManager.invalidateSheetData(spreadsheetId, sheetName);
          ULog.debug('リアクション更新後のキャッシュ無効化完了: ' + spreadsheetId);
        }
      } catch (cacheError) {
        ULog.warn('リアクション後のキャッシュ無効化エラー:', cacheError.message);
      }
    ...
    ULog.error('[ERROR]', 'リアクション処理エラー: ' + e.message);
    return createErrorResponse(e.message, 'リアクションの処理に失敗しました: ' + e.message);
  }
}
  ...
  try {
    ...
    try {
      ...
      if (userFolder) {
        ...
        ULog.debug('✅ フォームを作成と同時にユーザーフォルダに配置: %s', userFolder.getName());
      }
    } catch (folderMoveError) {
      ULog.warn('⚠️ フォーム作成直後の移動に失敗（後で再移動されます）:', folderMoveError.message);
    }
    ...
    try {
      ...
      ULog.warn('Email collection setting failed:', emailError.message);
    }
    ...
    ULog.error('[ERROR]', 'createFormFactory エラー:', error.message);
...
function addUnifiedQuestions(form, questionType, customConfig) {
  try {
    ...
    if (questionType === 'simple') {
      ...
      ULog.debug('addUnifiedQuestions - custom mode with config:', JSON.stringify(customConfig));
      ...
      // メイン質問
      var mainQuestionTitle = customConfig.mainQuestion
    ...
    ULog.debug('フォームに統一質問を追加しました: ' + questionType);
  } catch (error) {
    ULog.error('[ERROR]', 'addUnifiedQuestions エラー:', error.message);
    throw error;
  }
}
...
function doGetQuestionConfig() {
  try {
    ...
    ULog.error('[ERROR]', 'doGetQuestionConfig error:', error);
...
function saveClassChoices(userId, classChoices) {
  try {
    ...
    ULog.debug('クラス選択肢が保存されました:', classChoices);
    return createSuccessResponse(null, 'クラス選択肢が保存されました');
  } catch (error) {
    ULog.error('[ERROR]', 'クラス選択肢保存エラー:', error.message);
    return createErrorResponse('クラス選択肢の保存に失敗しました: ' + error.message, error.message);
  }
}
...
function getSavedClassChoices(userId) {
  try {
    ...
    var configJson = JSON.parse(userInfo.configJson || '{}');
    var savedClassChoices = configJson.savedClassChoices || [
      'クラス1',
      'クラス2',
      'クラス3',
      'クラス4',
    ...
    ULog.error('[ERROR]', 'クラス選択肢取得エラー:', error.message);
    return createErrorResponse(error.message, 'クラス選択肢の取得に失敗しました: ' + error.message, null);
  }
}
...
function createUnifiedForm(presetType, userEmail, userId, overrides = {}) {
  try {
    ...
    ULog.error('[ERROR]', `createUnifiedForm Error (${presetType}):`, error.message);
  ...
  try {
    ...
    try {
      ...
      if (userFolder) {
        ...
        ULog.debug(
          '✅ スプレッドシートを作成と同時にユーザーフォルダに配置: %s',
          userFolder.getName()
        );
      }
    } catch (folderMoveError) {
      ULog.warn(
        '⚠️ スプレッドシート作成直後の移動に失敗（後で再移動されます）:',
        folderMoveError.message
      );
    }
    ...
    try {
      ...
      ULog.debug('スプレッドシートを同一ドメイン閲覧可能に設定しました: ' + spreadsheetId);

      // 作成者（現在のユーザー）は所有者として保持
      ULog.debug('作成者は所有者として権限を保持: ' + userEmail);
    } catch (sharingError) {
      ULog.warn('共有設定の変更に失敗しましたが、処理を続行します: ' + sharingError.message);
    }
    ...
    try {
      ...
      if (!actualSheetName || actualSheetName === 'true' || actualSheetName.trim() === '') {
        ...
        ULog.warn(
          '不正なシート名が検出されました。デフォルトのシート名を使用します: ' + actualSheetName
        );
      }

      var sheetName = actualSheetName;
      ULog.info('✅ フォーム連携後の正確なシート名を取得:', sheetName);
    } catch (sheetNameError) {
      ULog.error('[ERROR]', '❌ フォーム連携後のシート名取得エラー:', sheetNameError.message);
      // フォールバック: 標準的なシート名を使用
      var sheetName = 'フォームの回答 1';
      ULog.warn('⚠️ シート名取得エラーのため、標準名を使用:', sheetName);
    }

    // サービスアカウントとスプレッドシートを共有（失敗しても処理継続）
    try {
      shareSpreadsheetWithServiceAccount(spreadsheetId);
      ULog.debug('サービスアカウントとの共有完了: ' + spreadsheetId);
    } catch (shareError) {
      ULog.warn('サービスアカウント共有エラー（処理継続）:', shareError.message);
    ...
    try {
      ...
      ULog.debug('✅ スプレッドシートURL取得成功:', spreadsheetUrl);
    } catch (urlError) {
      ULog.warn('⚠️ 初回URL取得失敗、リトライします:', urlError.message);
      ...
      for (let retry = 0; retry < 3; retry++) {
        try {
          ...
          if (spreadsheetUrl) {
            ULog.info(
              `✅ スプレッドシートURL取得成功（${retry + 1}回目のリトライ）:`,
              spreadsheetUrl
            );
            break;
          }
        } catch (retryError) {
          ULog.warn(`❌ リトライ${retry + 1}回目失敗:`, retryError.message);
        }
      }
      ...
      if (!spreadsheetUrl) {
        ...
        ULog.warn('⚠️ URLを手動構築しました:', spreadsheetUrl);
      }
    }
    ...
    ULog.error('[ERROR]', 'createLinkedSpreadsheet エラー:', error.message);
...
function shareAllSpreadsheetsWithServiceAccount() {
  try {
    ULog.debug('全スプレッドシートのサービスアカウント共有開始');
    ...
    for (var i = 0; i < allUsers.length; i++) {
      ...
      if (user.spreadsheetId && user.isActive === true) {
        try {
          ...
          successCount++;
          ULog.debug('共有成功:', user.adminEmail, user.spreadsheetId);
        } catch (shareError) {
          results.push({
            userId: user.userId,
            adminEmail: user.adminEmail,
          ...
          errorCount++;
          ULog.error('[ERROR]', '共有失敗:', user.adminEmail, shareError.message);
        }
      }
    }

    ULog.debug('全スプレッドシート共有完了:', successCount + '件成功', errorCount + '件失敗');
    ...
    ULog.error('[ERROR]', 'shareAllSpreadsheetsWithServiceAccount エラー:', error.message);
...
function repairUserSpreadsheetAccess(userEmail, spreadsheetId) {
  try {
    ULog.debug('スプレッドシートアクセス権限の修復を開始: ' + userEmail + ' -> ' + spreadsheetId);
    ...
    try {
      ...
      ULog.error('[ERROR]', 'DriveApp.getFileById error:', driveError.message);
    ...
    try {
      ...
      ULog.debug('スプレッドシートをドメイン全体で編集可能に設定しました');
    } catch (domainSharingError) {
      ULog.warn('ドメイン共有設定に失敗: ' + domainSharingError.message);

      // ドメイン共有に失敗した場合は個別にユーザーを追加
      try {
        file.addEditor(userEmail);
        ULog.debug('ユーザーを個別に編集者として追加しました: ' + userEmail);
      } catch (individualError) {
        ULog.error('[ERROR]', '個別ユーザー追加も失敗: ' + individualError.message);
      }
    }
    ...
    try {
      ...
      ULog.debug('SpreadsheetApp経由でユーザーを編集者として追加: ' + userEmail);
    } catch (spreadsheetAddError) {
      ULog.warn('SpreadsheetApp経由の追加で警告: ' + spreadsheetAddError.message);
    }
    ...
    if (serviceAccountEmail) {
      try {
        ...
        ULog.debug('サービスアカウントも編集者として追加: ' + serviceAccountEmail);
      } catch (serviceError) {
        ULog.warn('サービスアカウント追加で警告: ' + serviceError.message);
      }
    }

    return createSuccessResponse(null, 'スプレッドシートアクセス権限を修復しました。ドメイン全体でアクセス可能です。');
  } catch (e) {
    ULog.error('[ERROR]', 'スプレッドシートアクセス権限の修復に失敗: ' + e.message);
...
function addReactionColumnsToSpreadsheet(spreadsheetId, sheetName) {
  try {
    ...
    try {
      ...
      ULog.warn('Auto-resize failed:', resizeError.message);
    }

    ULog.debug('リアクション列を追加しました: ' + sheetName);
  } catch (e) {
    ULog.error('[ERROR]', 'リアクション列追加エラー: ' + e.message);
    // エラーでも処理は継続
  }
}
...
function getSheetData(userId, sheetName, classFilter, sortMode, adminMode) {
  // 統一Sheet管理システムを使用（互換性維持）
  return getSheetDataUnified(userId, sheetName, classFilter, sortMode, adminMode);
}
...
function executeGetSheetData(userId, sheetName, classFilter, sortMode) {
  try {
    ...
    ULog.debug('DEBUG: batchGetSheetsData responses: %s', JSON.stringify(responses));
    var sheetData = responses.valueRanges[0].values || [];
    ULog.debug('DEBUG: sheetData length: %s', sheetData.length);
    ...
    ULog.debug(
      '🔍 executeGetSheetData: シート設定取得完了 sheetKey=%s, hasGuessedConfig=%s',
      sheetKey,
      !!effectiveHeaderConfig.opinionHeader
    );
    ...
    if (!effectiveHeaderConfig.opinionHeader && headers.length > 1) {
      ...
      ULog.debug(
        '🔄 executeGetSheetData: フォールバック設定 - opinionHeader: %s',
        effectiveHeaderConfig.opinionHeader
      );
    }
    if (!effectiveHeaderConfig.reasonHeader && headers.length > 2) {
      ...
      ULog.debug(
        '🔄 executeGetSheetData: フォールバック設定 - reasonHeader: %s',
        effectiveHeaderConfig.reasonHeader
      );
    }

    // 統合デバッグ: 設定とデータの詳細ログ
    ULog.debug('🔍 executeGetSheetData: 統合デバッグ情報', {
      headers: headers,
    ...
    if (effectiveHeaderConfig.opinionHeader) {
      ...
      if (opinionIndex !== -1) {
        ...
        ULog.debug(
          '🎯 executeGetSheetData: AI判定結果からメインヘッダー設定 - %s',
          mainQuestionHeader
        );
      }
    }
    ...
    ULog.error('[ERROR]', 'シートデータ取得エラー: ' + e.message);
    ULog.error('[ERROR]', 'Error stack: ' + e.stack);
    ...
    try {
      ...
      ULog.warn('🔄 データ取得失敗 - フォールバック情報で応答: header=' + fallbackHeader);
      ...
      return createSuccessResponse({
        ...
        _fallbackUsed: true,
      }); // フロントエンドエラーを避けるためsuccessを返す
    } catch (fallbackError) {
      ULog.error('[ERROR]', 'フォールバック処理も失敗: ' + fallbackError.message);
...
function getSheetsList(userId) {
  try {
    ULog.debug('getSheetsList: Start for userId:', userId);
    var userInfo = findUserById(userId);
    if (!userInfo) {
      ULog.warn('getSheetsList: User not found:', userId);
      return [];
    }

    ULog.debug('getSheetsList: UserInfo found:', {
      userId: userInfo.userId,
      adminEmail: userInfo.adminEmail,
      spreadsheetId: userInfo.spreadsheetId,
      spreadsheetUrl: userInfo.spreadsheetUrl,
    ...
    if (!userInfo.spreadsheetId) {
      ULog.warn('getSheetsList: No spreadsheet ID for user:', userId);
      return [];
    }

    ULog.debug("getSheetsList: User's spreadsheetId:", userInfo.spreadsheetId);

    var service = getSheetsServiceCached();
    if (!service) {
      ULog.error('[ERROR]', '❌ getSheetsList: Sheets service not initialized');
      return [];
    }

    ULog.info('✅ getSheetsList: Service validated successfully');

    ULog.debug('getSheetsList: SheetsService obtained, attempting to fetch spreadsheet data...');

    var spreadsheet;
    try {
      spreadsheet = getSpreadsheetsData(service, userInfo.spreadsheetId);
    } catch (accessError) {
      var statusMatch = ((accessError && accessError.message) || '').match(
        /Sheets API error:\s*(\d+)/
      );
      ...
      if (statusCode === 403) {
        ...
        try {
          ...
          ULog.debug('getSheetsList: サービスアカウント権限を追加しました。再試行中...');

          // 少し待ってから再試行
          Utilities.sleep(1000);
          spreadsheet = getSpreadsheetsData(service, userInfo.spreadsheetId);
        } catch (repairError) {
          ULog.error('[ERROR]', 'getSheetsList: 権限修復に失敗:', repairError.message);
          ...
          try {
            ...
            if (currentUserEmail === userInfo.adminEmail) {
              ...
              ULog.debug('getSheetsList: ユーザー権限での修復を実行しました。');
            }
          } catch (finalRepairError) {
            ULog.error('[ERROR]', 'getSheetsList: 最終修復も失敗:', finalRepairError.message);
          }
    ...
    ULog.debug('getSheetsList: Raw spreadsheet data:', spreadsheet);
    if (!spreadsheet) {
      ULog.error('[ERROR]', 'getSheetsList: No spreadsheet data returned');
      return [];
    }
    ...
    if (!Array.isArray(spreadsheet.sheets)) {
      ULog.error(
        '[ERROR]',
        'getSheetsList: sheets property is not an array:',
        typeof spreadsheet.sheets
      );
      return [];
    }
    ...
    var sheets = spreadsheet.sheets
      .map(function (sheet) {
        if (!sheet.properties) {
          ULog.warn('getSheetsList: Sheet missing properties:', sheet);
          return null;
        }
        return {
    ...
    ULog.debug('getSheetsList: Successfully returning', sheets.length, 'sheets:', sheets);
    return sheets;
  } catch (e) {
    ULog.error('[ERROR]', 'getSheetsList: シート一覧取得エラー:', e.message);
    ULog.error('[ERROR]', 'getSheetsList: Error details:', e.stack);
    ULog.error('[ERROR]', 'getSheetsList: Error for userId:', userId);
    return [];
  }
}
...
function mapConfigToActualHeaders(configHeaders, actualHeaderIndices) {
  ...
  for (var configKey in configHeaders) {
    ...
    ULog.debug('mapConfigToActualHeaders: Trying to map %s = "%s"', configKey, configHeaderName);
    ...
    if (mappedIndex === undefined) {
      if (configKey === 'opinionHeader') {
        ...
        mappedIndex = 1;
        ULog.debug(
          'mapConfigToActualHeaders: Using fallback position for %s: index %s',
          configKey,
          mappedIndex
        );
  ...
  ULog.debug('mapConfigToActualHeaders: Final mapping result: %s', JSON.stringify(mappedIndices));
...
function getRowReactions(spreadsheetId, sheetName, rowIndex, userEmail) {
  try {
    ...
    ULog.error('[ERROR]', 'getRowReactions エラー: ' + e.message);
    return {
      UNDERSTAND: { count: 0, reacted: false },
      LIKE: { count: 0, reacted: false },
      CURIOUS: { count: 0, reacted: false },
...
function updateIsActiveStatus(requestUserId, isActive) {
  ...
  try {
    ...
    if (!activeUserEmail) {
      return createErrorResponse('USER_NOT_AUTHENTICATED', 'ユーザーが認証されていません');
    }

    // 現在のユーザー情報を取得
    var userInfo = findUserByEmail(activeUserEmail);
    if (!userInfo) {
      return createErrorResponse('USER_NOT_FOUND', 'ユーザーがデータベースに登録されていません');
    }

    // コア統一関数を使用
    var result = updateUserActiveStatusCore(userInfo.userId, isActive, {
      callerType: 'api',
      clearCache: true,
    });
    ...
    if (result.success) {
      ...
      return createSuccessResponse({ newStatus: isActive }, statusMessage);
    } else {
      return createErrorResponse('DATABASE_UPDATE_FAILED', result.message);
    }
  } catch (e) {
    ULog.error('[ERROR]', 'updateIsActiveStatus エラー: ' + e.message);
    return createErrorResponse(e.message, 'isActive状態の更新に失敗しました');
  }
}
...
function hasSetupPageAccess() {
  try {
    ...
    ULog.error('[ERROR]', 'hasSetupPageAccess エラー: ' + e.message);
    return false;
  }
}
...
function shouldEnableDebugMode() {
  try {
    ...
    ULog.warn('shouldEnableDebugMode error:', error.message);
    return false;
  }
}
...
function isSystemAdmin() {
  try {
    ...
    ULog.error('[ERROR]', 'isSystemAdmin エラー: ' + e.message);
    return false;
  }
}
...
async function deleteUserAccountByAdminForUI(targetUserId, reason) {
  try {
    ...
    ULog.error('[ERROR]', 'deleteUserAccountByAdmin wrapper error:', error.message);
...
function getDeletionLogsForUI() {
  try {
    ...
    return createSuccessResponse(logs, 'ログ取得に成功しました');
  } catch (error) {
    ULog.error('[ERROR]', 'getDeletionLogs wrapper error:', error.message);
...
function getDiagnosticLogsForUI(limit) {
  try {
    ...
    ULog.error('[ERROR]', 'getDiagnosticLogs wrapper error:', error.message);
    return createErrorResponse(error.message);
  }
}
...
function getAllUsersForAdminForUI(requestUserId) {
  try {
    ...
    return createSuccessResponse(result, 'ユーザー一覧取得に成功しました');
  } catch (error) {
    ULog.error('[ERROR]', 'getAllUsersForAdminForUI wrapper error:', error.message);
    return createErrorResponse(error.message);
  }
}
...
function updateUserActiveStatusForUI(targetUserId, isActive) {
  try {
    if (!targetUserId) {
      return createErrorResponse('ユーザーIDが指定されていません', 'USER_ID_REQUIRED');
    }

    // コア統一関数を使用
    var result = updateUserActiveStatusCore(targetUserId, isActive, {
      callerType: 'ui',
      clearCache: true,
    });
    ...
    if (result.success) {
      ...
      return createErrorResponse(result.message, result.error || 'DATABASE_UPDATE_FAILED');
    }
  } catch (error) {
    ULog.error('[ERROR]', 'updateUserActiveStatusForUI エラー:', error);
...
function customSetup(requestUserId, config) {
  verifyUserAccess(requestUserId);
  try {
    ULog.debug('🎨 CustomSetup: シンプルセットアップ開始', { requestUserId, config });
    ...
    // ステップ3: AI列判定の実行
    ULog.debug('🤖 CustomSetup: AI列判定実行中...');
    const aiDetectionResult = performAutoAIDetection(
      requestUserId,
      formAndSsInfo.spreadsheetId,
      formAndSsInfo.sheetName
    );
    ULog.debug('✅ CustomSetup: AI列判定完了', aiDetectionResult);
    ...
    ULog.debug('💾 CustomSetup: データベース更新中...');
    ...
    ULog.debug('✅ CustomSetup: データベース更新成功');

    // ステップ5: キャッシュのクリア
    ULog.debug('🔄 CustomSetup: キャッシュクリア中...');
    clearExecutionUserInfoCache();
    invalidateUserCache(requestUserId, userEmail, formAndSsInfo.spreadsheetId, true);
    ULog.debug('✅ CustomSetup: キャッシュクリア完了');
    ...
    ULog.error('[ERROR]', '❌ customSetup エラー: ' + e.message);
...
function createCustomFormAndSheet(userEmail, requestUserId, config) {
  ...
  if (config.sheetName && config.sheetName !== formAndSsInfo.sheetName) {
    try {
      ...
      ULog.warn('シート名の変更に失敗しました: ' + e.message);
    }
  }
...
function performAutoAIDetection(requestUserId, spreadsheetId, sheetName) {
  try {
    ULog.debug('🤖 AI列判定自動実行開始', { requestUserId, spreadsheetId, sheetName });
    ...
    ULog.error('[ERROR]', '❌ AI列判定自動実行エラー', { error: error.message });
    return {
      success: false,
      aiDetected: false,
...
function createCustomFormUI(requestUserId, config) {
  try {
    ...
    ULog.debug('createCustomFormUI - converted config:', JSON.stringify(convertedConfig));
    ...
    if (result.sheetName) {
      try {
        ULog.debug('🎯 カスタムフォーム作成後のシートアクティベーション開始:', result.sheetName);
        const activeSheetResult = setActiveSheet(requestUserId, result.sheetName);
        ULog.info('✅ カスタムフォーム作成後のシートアクティベーション完了:', activeSheetResult);
      } catch (sheetError) {
        ULog.warn('⚠️ シートアクティベーション失敗（処理継続）:', sheetError.message);
        // シートアクティベーション失敗時のログ詳細化
        ULog.error('[ERROR]', 'シートアクティベーション失敗詳細:', {
          requestUserId: requestUserId,
      ...
      ULog.warn('⚠️ createUnifiedForm結果にsheetNameが含まれていません:', result);
    }
    ...
    try {
      ULog.debug('📁 ユーザーフォルダ作成とファイル移動開始...');
      ...
      if (folder) {
        ...
        try {
          ...
          ULog.warn('カスタムフォームファイル移動エラー:', formMoveError.message);
        }
        ...
        try {
          ...
          ULog.warn('カスタムスプレッドシートファイル移動エラー:', ssMoveError.message);
        }

        ULog.debug('📁 カスタムセットアップファイル移動結果:', moveResults);
      }
    } catch (folderError) {
      ULog.warn('カスタムセットアップフォルダ処理エラー:', folderError.message);
    }

    // 既存ユーザーの情報を更新（スプレッドシート情報を追加）
    const existingUser = findUserById(requestUserId);
    if (existingUser) {
      ULog.debug('createCustomFormUI - updating user data for:', requestUserId);
      ...
      if (result.spreadsheetId && typeof result.spreadsheetId === 'string') {
        ...
        ULog.info('✅ publishedSpreadsheetId設定完了:', result.spreadsheetId);
      } else {
        ULog.error('[ERROR]', '❌ 無効なspreadsheetId:', result.spreadsheetId);
        throw new Error('フォーム作成は成功しましたが、スプレッドシートIDが無効です');
      }

      if (
        result.sheetName &&
        typeof result.sheetName === 'string' &&
        result.sheetName.trim() !== '' &&
        result.sheetName !== 'true'
      ) {
        updatedConfigJson.publishedSheetName = result.sheetName;
        ULog.info('✅ publishedSheetName設定完了:', result.sheetName);
      } else {
        ULog.error('[ERROR]', '❌ 無効なsheetName:', result.sheetName);
      ...
      ULog.debug('createCustomFormUI - update data:', JSON.stringify(updateData));

      updateUser(requestUserId, updateData);

      // カスタムフォーム作成後の包括的キャッシュ同期（Quick Startと同様）
      ULog.debug('🗑️ カスタムフォーム作成後の包括的キャッシュ同期中...');
      ...
      ULog.warn('createCustomFormUI - user not found:', requestUserId);
    }
    ...
    ULog.error('[ERROR]', 'createCustomFormUI error:', error.message);
    return {
      success: false,
      message: error.message,
    };
...
function createQuickStartFormUI(requestUserId) {
  try {
    ...
    if (result.sheetName) {
      try {
        ULog.debug('🎯 QuickStart作成後のシートアクティベーション開始:', result.sheetName);
        const activeSheetResult = setActiveSheet(requestUserId, result.sheetName);
        ULog.info('✅ QuickStart作成後のシートアクティベーション完了:', activeSheetResult);
      } catch (sheetError) {
        ULog.warn('⚠️ QuickStartシートアクティベーション失敗（処理継続）:', sheetError.message);
      }
    } else {
      ULog.warn('⚠️ QuickStart createUnifiedForm結果にsheetNameが含まれていません:', result);
    }
    ...
    if (existingUser) {
      ...
      if (result.spreadsheetId && typeof result.spreadsheetId === 'string') {
        updatedConfigJson.publishedSpreadsheetId = result.spreadsheetId;
        ULog.info('✅ QuickStart publishedSpreadsheetId設定完了:', result.spreadsheetId);
      } else {
        ULog.error('[ERROR]', '❌ QuickStart 無効なspreadsheetId:', result.spreadsheetId);
      }
      ...
      if (
        ...
        updatedConfigJson.publishedSheetName = result.sheetName;
        ULog.info('✅ QuickStart publishedSheetName設定完了:', result.sheetName);
      } else {
        ULog.error('[ERROR]', '❌ QuickStart 無効なsheetName:', result.sheetName);
      }
    ...
    ULog.error('[ERROR]', 'createQuickStartFormUI error:', error.message);
    return {
      success: false,
      message: error.message,
    };
...
async function deleteCurrentUserAccount(requestUserId) {
  try {
    ...
    ULog.error('[ERROR]', 'deleteCurrentUserAccount error:', error.message);
    return {
      success: false,
      message: error.message,
    };
...
function activateSheetSimple(requestUserId, sheetName) {
  try {
    ...
    ULog.error('[ERROR]', 'activateSheetSimple error:', error.message);
    return {
      success: false,
      message: error.message,
    };
...
function getLoginStatus() {
  ...
  try {
    ...
    ULog.debug('getLoginStatus: 高信頼性検索開始:', activeUserEmail);
    ...
    try {
      ...
      if (cached) {
        ...
        if (cacheAge < 10000) {
          // 10秒以内のキャッシュのみ使用
          ULog.debug('getLoginStatus: 新鮮なキャッシュ使用:', parsedCache.status);
          return parsedCache;
        } else {
          ULog.debug('getLoginStatus: キャッシュが古いためスキップ');
        }
      }
    } catch (cacheError) {
      ULog.debug('getLoginStatus: キャッシュ読み込みエラー:', cacheError.message);
    }
    ...
    try {
      ...
      if (userInfo) {
        searchSuccess = true;
        ULog.debug('getLoginStatus: Stage 2成功 - キャッシュ検索でユーザー発見');
      }
    } catch (stage2Error) {
      searchAttempts.push({ method: 'findUserByEmail', error: stage2Error.message });
      ULog.warn('getLoginStatus: Stage 2失敗:', stage2Error.message);
    }

    // Stage 3: 強制フレッシュ検索（データベース直接）
    if (!searchSuccess) {
      try {
        ULog.debug('getLoginStatus: Stage 3開始 - 強制フレッシュ検索');
        ...
        if (userInfo) {
          searchSuccess = true;
          ULog.debug('getLoginStatus: Stage 3成功 - 強制フレッシュでユーザー発見');
        }
      } catch (stage3Error) {
        searchAttempts.push({
          method: 'fetchUserFromDatabase_forceFresh',
          error: stage3Error.message,
        });
        ULog.warn('getLoginStatus: Stage 3失敗:', stage3Error.message);
      }
    }

    // Stage 4: 最終確認検索（複数方法で確認）
    if (!searchSuccess) {
      ULog.debug('getLoginStatus: Stage 4開始 - 最終確認検索');
      const finalMethods = [
      ...
      for (let i = 0; i < finalMethods.length && !searchSuccess; i++) {
        try {
          ...
          if (userInfo) {
            searchSuccess = true;
            ULog.debug('getLoginStatus: Stage 4成功:', methodName);
            break;
          }
        } catch (finalError) {
    ...
    if (userInfo && searchSuccess) {
      // ユーザーが見つかった場合の状態判定
      const isActive = Boolean(userInfo.isActive);
      ...
      if (isActive) {
        ...
        ULog.info('✅ getLoginStatus: アクティブユーザー確認完了', {
        ...
        ULog.warn('⚠️ getLoginStatus: 非アクティブユーザー検出', {
          email: activeUserEmail,
          userId: userInfo.userId,
        });
      }
      ...
      ULog.warn('⚠️ getLoginStatus: ユーザーが見つかりません', {
        email: activeUserEmail,
    ...
    ULog.error('[ERROR]', '❌ getLoginStatus 重大エラー:', {
      error: error.message,
...
function confirmUserRegistration() {
  ...
  try {
    ...
    ULog.info('confirmUserRegistration: 統合登録確認開始', { email: activeUserEmail });
    ...
    try {
      ...
      ULog.debug('confirmUserRegistration: 関連キャッシュクリア完了');
    } catch (cacheError) {
      // キャッシュクリアの失敗は無視
    }
    ...
    const searchStages = [
      ...
      {
        name: 'forceFresh',
        method: () => fetchUserFromDatabase('adminEmail', activeUserEmail, { forceFresh: true }),
      },
      {
        name: 'retryFresh',
        method: () => {
          Utilities.sleep(500); // 短い待機後再検索
          return fetchUserFromDatabase('adminEmail', activeUserEmail, {
            forceFresh: true,
            retryOnce: true,
          });
        },
    ...
    for (const stage of searchStages) {
      ...
      try {
        ULog.debug('confirmUserRegistration: 検索ステージ実行:', stage.name);
        existingUser = stage.method();
        if (existingUser && existingUser.userId && existingUser.adminEmail) {
          userFound = true;
          ULog.info('✅ confirmUserRegistration: 既存ユーザー発見', {
            stage: stage.name,
            userId: existingUser.userId,
            email: activeUserEmail,
          });
        ...
        ULog.warn('confirmUserRegistration: 検索ステージエラー', {
          stage: stage.name,
          error: stageError.message,
        });
      }
    ...
    if (existingUser && userFound) {
      ...
      const elapsedTime = Date.now() - startTime;
      ULog.info('✅ confirmUserRegistration: 既存ユーザー確認完了', {
        email: activeUserEmail,
        userId: existingUser.userId,
        isActive: isActive,
        elapsed: elapsedTime + 'ms',
    ...
    // 新規ユーザー登録の実行
    ULog.debug('confirmUserRegistration: 新規ユーザー登録を開始');
    ...
    try {
      ...
      if (registrationResult && registrationResult.userId) {
        ...
        try {
          ...
          if (!verificationUser) {
            ULog.warn(
              'confirmUserRegistration: 登録後検証で見つからない:',
              registrationResult.userId
            );
          } else {
            ULog.debug('✅ confirmUserRegistration: 登録後検証成功');
          }
        } catch (verifyError) {
          ULog.warn('confirmUserRegistration: 登録後検証でエラー:', verifyError.message);
        }
      ...
      ULog.info('✅ confirmUserRegistration: 新規ユーザー登録完了', {
        email: activeUserEmail,
        userId: registrationResult?.userId,
      ...
      const totalElapsed = Date.now() - startTime;
      ULog.error('[ERROR]', '❌ confirmUserRegistration: 登録処理エラー', {
        email: activeUserEmail,
    ...
    const totalElapsed = Date.now() - startTime;
    ULog.error('[ERROR]', '❌ confirmUserRegistration: 重大エラー', {
      email: activeUserEmail || 'unknown',
      error: error.message,
      elapsed: totalElapsed + 'ms',
    });
...
function getInitialData(requestUserId, targetSheetName) {
  ULog.debug('🚀 getInitialData: 統合初期化開始', { requestUserId, targetSheetName });
  ...
  try {
    ...
    try {
      ...
      ULog.warn('⚠️ データ整合性チェック中にエラー:', consistencyError.message);
      // エラーが発生しても初期化処理は続行
    }
    ...
    try {
      ...
      ULog.warn('Answer count retrieval failed:', err.message);
    }
    ...
    try {
      ...
      ULog.debug('setupStep決定完了', { userId: currentUserId, setupStep: setupStep });
    } catch (stepError) {
      ULog.warn('setupStep決定でエラー、デフォルト値(1)を使用:', stepError.message);
      setupStep = 1;
    }
    ...
    if (!includeSheetDetails && userInfo.spreadsheetId && configJson) {
      ULog.warn('⚠️ シート名が指定されていません。デフォルトシート名を検索中...');
      try {
        // 一般的なシート名パターンを試す
        ...
        if (spreadsheetInfo && spreadsheetInfo.sheets && spreadsheetInfo.sheets.length > 0) {
          ...
          for (const commonName of commonSheetNames) {
            if (spreadsheetInfo.sheets.some((sheet) => sheet.properties.title === commonName)) {
              ...
              ULog.info('✅ フォールバックシート名を使用:', commonName);
              break;
            }
          }
          ...
          if (!includeSheetDetails) {
            ...
            ULog.info('✅ 最初のシートを使用:', includeSheetDetails);
          }
        }
      } catch (fallbackError) {
        ULog.warn('⚠️ フォールバックシート名検索に失敗:', fallbackError.message);
      }
    }
    ...
    if (includeSheetDetails && userInfo.spreadsheetId) {
      try {
        ...
        var sheetDetails = getSheetDetailsFromContext(
          context,
          userInfo.spreadsheetId,
          includeSheetDetails
        );
        ...
        ULog.info('✅ シート詳細を統合応答に追加 (最適化版):', includeSheetDetails);
        // getInitialData内で生成したcontextの変更をコミット
        commitAllChanges(context);
      } catch (sheetErr) {
        ULog.warn('Sheet details retrieval failed:', sheetErr.message);
    ...
    ULog.debug('🎯 getInitialData: 統合初期化完了', {
    ...
    try {
      ...
      ULog.error('[ERROR]', '❌ Fallback userInfo creation failed:', userError);
    }
...
function fixDataConsistencyManual(requestUserId) {
  try {
    verifyUserAccess(requestUserId);
    ULog.debug('🔧 手動データ整合性修正実行:', requestUserId);
    ...
    ULog.error('[ERROR]', '❌ 手動データ整合性修正エラー:', error);
    return {
      success: false,
...
function getApplicationStatusForUI() {
  try {
    ...
    ULog.error('[ERROR]', 'getApplicationStatusForUI エラー:', error);
    return {
      success: false,
      message: error.message,
    };
...
function setApplicationStatusForUI(enabled) {
  try {
    ...
    ULog.error('[ERROR]', 'setApplicationStatusForUI エラー:', error);
    return {
      success: false,
      message: error.message,
    };
...
function testForceLogoutRedirect() {
  try {
    ULog.debug('🧪 testForceLogoutRedirect called');
    return {
      success: true,
    ...
    ULog.error('[ERROR]', 'testForceLogoutRedirect エラー:', error);
    return {
      success: false,
      message: error.message,
      timestamp: new Date().toISOString(),
...
function getFlowProgress(flowId) {
  try {
    ULog.debug(`🔍 getFlowProgress called for flowId: ${flowId}`);
...
function getSpreadsheetCreatedDate(spreadsheetId) {
  try {
    if (!spreadsheetId) {
      ULog.warn('⚠️ getSpreadsheetCreatedDate: spreadsheetIdが指定されていません');
      return null;
    }
    ...
    ULog.warn('⚠️ スプレッドシート作成日取得失敗:', error.message);
    return null;
  }
}
...
function getSpreadsheetCreatedDateAPI(requestUserId) {
  try {
    ...
    ULog.error('[ERROR]', '❌ getSpreadsheetCreatedDateAPI エラー:', error);
    return {
      success: false,
...
function updateUserAPI(requestUserId, updateData) {
  try {
    ...
    if (filteredUpdateData.spreadsheetId) {
      ULog.debug('💾 Clearing caches after spreadsheet switch:', filteredUpdateData.spreadsheetId);
      ...
      try {
        ...
        ULog.warn('Cache invalidation warning:', cacheError.message);
      }
    }
    ...
    ULog.error('[ERROR]', '❌ updateUserAPI エラー:', error);
    return {
      success: false,
