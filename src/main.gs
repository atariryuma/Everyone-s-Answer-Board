...
/**
 * Webアプリケーションのメインエントリーポイント - 500エラー完全防止版
 ...
 * レベル2: 安全モード実行フロー
 */
function safeModeExecutionFlow(e) {
  const params = extractParametersRobustly(e);
  console.log('🛡️ Safe mode routing:', params);

  // 必須機能のみで動作
  switch (params.mode) {
    case 'admin':
      return handleAdminModeRobustly(params);
    case 'view':
      return handleViewModeRobustly(params);
    case 'login':
      return createSafeLoginPage();
    case 'appSetup':
      return createSafeAppSetupPage();
    default:
      return handleDefaultModeRobustly();
  }
}
 ...
 * レベル3: 緊急モード実行フロー
 */
function emergencyModeExecutionFlow(e, error) {
  ...
  try {
    if (e && e.parameter) {
      mode = e.parameter.mode || 'emergency';
      userId = e.parameter.userId || null;
    }
  } catch (extractError) {
    console.debug('Parameter extraction failed in emergency mode:', extractError);
  }
  
  return createEmergencyResponsePage(error, mode, userId);
}

/**
 * 堅牢なパラメータ抽出
 */
function extractParametersRobustly(e) {
  const defaults = {
    mode: 'default',
    userId: null,
    setupParam: null
  ...
  try {
    ...
    return {
      mode: sanitizeString(e.parameter.mode) || 'default',
      userId: sanitizeString(e.parameter.userId) || null,
      setupParam: sanitizeString(e.parameter.setupParam) || null
    };
  } catch (error) {
 ...
 * 堅牢な管理モードハンドラー
 */
function handleAdminModeRobustly(params) {
  try {
    if (!params.userId) {
      return createRequiredParameterErrorPage('ユーザーID');
    }
    
    // 現在のユーザーを安全に取得
    const currentUser = getCurrentUserSafely();
    if (!currentUser) {
      return createAuthenticationRequiredPage();
    }
    
    // データベース接続を確認
    const dbStatus = checkDatabaseConnectionSafely();
    if (!dbStatus.connected) {
      return createDatabaseSetupRequiredPage(dbStatus.error);
    }
    ...
    // フォールバック：基本管理パネル
    return createBasicAdminPanelRobust(params.userId, currentUser);
    
  } catch (error) {
    console.error('Admin mode robust handling failed:', error);
    return createAdminModeErrorPage(error, params.userId);
  }
}
 ...
 * 堅牢なビューモードハンドラー
 */
function handleViewModeRobustly(params) {
  try {
    if (!params.userId) {
      return createRequiredParameterErrorPage('ユーザーID');
    }
    ...
    return createBasicViewPageRobust(params.userId);
  } catch (error) {
    console.error('View mode robust handling failed:', error);
    return createViewModeErrorPage(error, params.userId);
  }
}
 ...
 * 堅牢なデフォルトモードハンドラー
 */
function handleDefaultModeRobustly() {
  try {
    const currentUser = getCurrentUserSafely();
    if (!currentUser) {
      return createSafeLoginPage();
    ...
    if (typeof handleDefaultRoute === 'function') {
      return handleDefaultRoute();
    }
    
    return createWelcomePageRobust(currentUser);
  } catch (error) {
    console.error('Default mode robust handling failed:', error);
    return createSafeLoginPage('システムエラーが発生しました');
  }
}
...
function getCurrentUserSafely() {
  try {
    ...
    console.debug('Failed to get current user:', error);
    return null;
  }
}

function checkDatabaseConnectionSafely() {
  try {
    const properties = PropertiesService.getScriptProperties();
    ...
    if (!dbId) {
      return {
        connected: false,
        error: 'DATABASE_SPREADSHEET_ID not configured'
      };
    }
 ...
 * 安全なページ生成関数群
 */
function createSafeLoginPage(message = '') {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0f172a; color: white; min-height: 100vh; display: flex; align-items: center; justify-content: center; }
    .container { background: #1e293b; padding: 2rem; border-radius: 1rem; max-width: 400px; width: 90%; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); }
    h1 { font-size: 1.5rem; margin-bottom: 1rem; text-align: center; color: #3b82f6; }
    .message { background: #374151; padding: 1rem; border-radius: 0.5rem; margin-bottom: 1rem; font-size: 0.9rem; }
    .button { background: #3b82f6; color: white; padding: 0.75rem 1.5rem; border: none; border-radius: 0.5rem; cursor: pointer; width: 100%; font-size: 1rem; font-weight: 500; }
...
<body>
  <div class="container">
    ...
    <p style="text-align: center; margin-bottom: 1.5rem; color: #94a3b8;">Googleアカウントでログインしてください</p>
    <button class="button" onclick="performLogin()">Googleでログイン</button>
  </div>
  <script>
    function performLogin() {
      window.location.href = window.location.origin + window.location.pathname + '?mode=default';
    }
  </script>
</body>
</html>`;
...
function createUltimateFallbackPage(error, requestData) {
  const html = `<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>システム復旧モード</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: Arial, sans-serif; background: #000; color: #fff; padding: 20px; line-height: 1.6; }
    .container { max-width: 800px; margin: 0 auto; }
    .emergency { background: #dc2626; padding: 20px; border-radius: 8px; margin-bottom: 20px; text-align: center; }
    .info { background: #1f2937; padding: 20px; border-radius: 8px; margin-bottom: 20px; }
    ...
    .button:hover { background: #2563eb; }
    .code { background: #374151; padding: 15px; border-radius: 6px; font-family: monospace; font-size: 12px; margin: 10px 0; }
    .actions { display: flex; flex-wrap: wrap; gap: 10px; justify-content: center; margin: 20px 0; }
  </style>
</head>
<body>
  <div class="container">
    <div class="emergency">
      <h1>🚨 システム復旧モード</h1>
      <p>システムに重大な問題が発生しました。以下の復旧手順をお試しください。</p>
    </div>
    
    <div class="info">
      <h2>🔧 推奨される復旧手順</h2>
      <ol>
        <li><strong>ページを再読み込み</strong> - 一時的な問題の可能性があります</li>
        <li><strong>数分待ってから再アクセス</strong> - システムが自動修復中の可能性があります</li>
        <li><strong>ブラウザのキャッシュをクリア</strong> - 古いデータが原因の場合があります</li>
        <li><strong>管理者に問い合わせ</strong> - 問題が継続する場合</li>
      </ol>
    </div>
    
    <div class="actions">
      <button class="button" onclick="location.reload()">🔄 再読み込み</button>
      <button class="button" onclick="clearCache()">🗑️ キャッシュクリア</button>
      <button class="button" onclick="goToSetup()">⚙️ セットアップ</button>
      <button class="button" onclick="goToLogin()">🔑 ログイン</button>
    </div>
    
    <div class="info">
      <h3>📋 エラー詳細</h3>
      <div class="code">
        タイムスタンプ: ${new Date().toISOString()}<br>
        エラー: ${escapeHtml(error ? error.toString() : 'Unknown error')}<br>
        リクエストID: ${generateRequestId()}<br>
        パラメータ: ${escapeHtml(requestData && requestData.parameter ? JSON.stringify(requestData.parameter) : 'None')}
      </div>
    </div>
    
    <div class="info" style="text-align: center; background: #059669;">
      <h3>✅ 自動修復機能</h3>
      <p>システムは自動的に問題の修復を試行しています。数分後に正常に動作する可能性があります。</p>
    </div>
  </div>
  ...
  <script>
    function clearCache() {
      try {
        ...
        alert('キャッシュのクリアに失敗しましたが、ページを再読み込みします。');
        location.reload();
      }
    }
    ...
    // エラー情報をコンソールに出力
    console.error('Ultimate Fallback Page Activated');
    console.error('Error Details:', {
      error: ${JSON.stringify(error ? error.toString() : 'Unknown')},
      timestamp: '${new Date().toISOString()}',
...
// 追加のエラーページ生成関数（簡略版）
function createRequiredParameterErrorPage(paramName) {
  return createSafeLoginPage(`必須パラメータ「${paramName}」が不足しています`);
}

function createAuthenticationRequiredPage() {
  return createSafeLoginPage('認証が必要です');
}

function createDatabaseSetupRequiredPage(error) {
  return createSafeLoginPage(`データベースの設定が必要です: ${error}`);
}

function createAdminModeErrorPage(error, userId) {
  return createSafeLoginPage(`管理モードでエラーが発生しました: ${error}`);
}

function createViewModeErrorPage(error, userId) {
  return createSafeLoginPage(`ビューモードでエラーが発生しました: ${error}`);
}

function createBasicAdminPanelRobust(userId, userEmail) {
  return createSafeLoginPage(`管理パネルを準備中です。ユーザー: ${userEmail}`);
}

function createBasicViewPageRobust(userId) {
  return createSafeLoginPage(`ビューページを準備中です。`);
}

function createWelcomePageRobust(userEmail) {
  return createSafeLoginPage(`ようこそ、${userEmail}さん`);
}

function createSafeAppSetupPage() {
  return createSafeLoginPage('アプリセットアップページを準備中です');
}
