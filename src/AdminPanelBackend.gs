...
function connectToDataSource(spreadsheetId, sheetName) {
  try {
    ...
    const userInfo = findUserByEmail(currentUser);
...
/**
 * システムメトリクスを取得
 * @returns {Object} システム状態情報
 */
function getSystemMetrics() {
  try {
    console.log('getSystemMetrics: システムメトリクス取得開始');
    
    // 既存のcoreGetAllUsers関数を活用
    let activeUsers = 0;
    let activeBoards = 0;
    
    try {
      const allUsers = coreGetAllUsers();
      activeUsers = allUsers ? allUsers.length : 0;
      
      // アクティブなボードを数える（簡易実装）
      activeBoards = countActiveBoards();
    ...
    console.log('getSystemMetrics: メトリクス取得完了', metrics);
    return metrics;
    
  } catch (error) {
    console.error('getSystemMetrics エラー:', error);
    return {
      activeUsers: 0,
      activeBoards: 0,
      systemHealth: 'error',
...
/**
 * アクティブなボード数を数える
 * @returns {number} アクティブなボード数
 */
function countActiveBoards() {
  try {
    // PropertiesServiceから公開済みアプリの情報を取得
    const props = PropertiesService.getScriptProperties();
    const allProps = props.getProperties();
    
    let activeCount = 0;
    Object.keys(allProps).forEach(key => {
      if (key.includes('_published') && allProps[key] === 'true') {
        activeCount++;
      }
    });
    ...
    console.error('countActiveBoards エラー:', error);
    return 0;
  }
}
