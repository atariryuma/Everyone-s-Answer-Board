    ...
    return {
      ...
      response: {
        ...
        unified: (success, data = null, message = null, error = null) =>
          createUnifiedResponse(success, data, message, error),
      },
      ...
      key: {
        ...
        sheetDataCache: (operation, spreadsheetId, params) =>
          `sheetData:${operation}:${spreadsheetId}:${JSON.stringify(params)}`,
      },
       ...
       */
      form: {
        unified: (type, userEmail, userId, overrides) =>
          createUnifiedForm(type, userEmail, userId, overrides),
        ...
        forSpreadsheet: (spreadsheetId, sheetName) =>
          createFormForSpreadsheet(spreadsheetId, sheetName),
        create: (type, options) => {
          switch (type) {
            case 'quickstart':
            case 'custom':
