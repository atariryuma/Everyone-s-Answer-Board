...
//   2 回目以降は parse コストが発生しない。
const DATA_APIS_SOURCE = fs.readFileSync(path.resolve(__dirname, '../src/DataApis.js'), 'utf8');
const DATA_APIS_SCRIPT = new vm.Script(DATA_APIS_SOURCE, { filename: 'DataApis.js' });
...
function loadCtx(overrides = {}) {
  ...
  DATA_APIS_SCRIPT.runInContext(ctx);
  return ctx;
}
