...
import type { GoogleAppsScript } from 'google-apps-script';
...
const createSpreadsheetAppMock = () => {
  const mockSpreadsheet = (id: string) => ({
    getId: jest.fn(() => id),
    getName: jest.fn(() =>
      mockDataStore.getValue(`spreadsheet_${id}_name`, `Test Spreadsheet ${id}`)
    ),
    ...
    deleteSheet: jest.fn((sheet: any) => {
      ...
      const currentSheets = mockDataStore.getValue('sheet_names', ['Sheet1']);
      mockDataStore.setValue(
        'sheet_names',
        currentSheets.filter((name: string) => name !== sheetName)
      );
    }),
  });
  ...
  const createMockSheet = (name: string) => {
    const getStoredData = (rowStart = 1, colStart = 1, numRows = 10, numCols = 5) => {
      ...
      const defaultData = Array(numRows)
        .fill(null)
    ...
    return {
      ...
      getRange: jest.fn((row: number, col: number, numRows = 1, numCols = 1) => {
        ...
        if (row < 1 || col < 1) {
          throw new GASError(
            'Range row and column indices must be positive integers',
            'INVALID_ARGUMENT'
          );
        }
  ...
  const createMockRange = (
    ...
    return {
      ...
      getValue: jest.fn(() => {
        ...
        const values = (
          createMockRange(sheetName, row, col, numRows, numCols).getValues as jest.Mock
        )();
        return values[0][0];
      }),
      ...
      clear: jest.fn(() => {
        ...
        const emptyValues = Array(numRows)
          .fill(null)
          .map(() => Array(numCols).fill(''));
        (createMockRange(sheetName, row, col, numRows, numCols).setValues as jest.Mock)(
          emptyValues
        );
      }),
    };
...
export const MockTestUtils = {
   ...
   */
  simulateError: (condition: string, error: Error) =>
    mockDataStore.setErrorCondition(condition, error),
  ...
  simulatePermissionError: (resource: string) => {
    mockDataStore.setErrorCondition(
      resource,
      new GASError('Permission denied', 'PERMISSION_DENIED')
    );
  },
};
...
export { SpreadsheetApp, PropertiesService, MockTestUtils, GASError };
