...
test('extractFieldValueUnified: extracts value using columnMapping', () => {
  const ctx = loadColumnMappingContext();
  const headers = ['タイムスタンプ', '名前', '回答'];
  const row = ['2026-04-19', '田中', 'これが私の回答です'];

  const result = ctx.extractFieldValueUnified(row, headers, 'answer', { answer: 2 });

  assert.equal(result.value, 'これが私の回答です');
  assert.equal(result.index, 2);
  assert.equal(result.confidence, 100);
});

test('extractFieldValueUnified: falls back to auto-detection when mapping absent', () => {
  const ctx = loadColumnMappingContext();
  const headers = ['タイムスタンプ', '名前', '回答'];
  const row = ['2026-04-19', '田中', '自動検出された回答'];

  const result = ctx.extractFieldValueUnified(row, headers, 'answer');

  assert.equal(result.value, '自動検出された回答');
  assert.equal(result.index, 2);
});

test('extractFieldValueUnified: returns null for empty cell', () => {
  const ctx = loadColumnMappingContext();
  const headers = ['タイムスタンプ', '名前', '回答'];
  const row = ['2026-04-19', '田中', ''];

  const result = ctx.extractFieldValueUnified(row, headers, 'answer', { answer: 2 });

  assert.equal(result.value, null);
  assert.equal(result.method, 'empty_value');
});

test('extractFieldValueUnified: returns null for whitespace-only cell', () => {
  const ctx = loadColumnMappingContext();
  const headers = ['名前', '回答'];
  const row = ['田中', '   '];

  const result = ctx.extractFieldValueUnified(row, headers, 'answer', { answer: 1 });

  assert.equal(result.value, null);
  assert.equal(result.method, 'empty_value');
});

test('extractFieldValueUnified: returns not_found when index out of range', () => {
  const ctx = loadColumnMappingContext();
  const headers = ['名前', '回答'];
  const row = ['田中']; // row shorter than mapping

  const result = ctx.extractFieldValueUnified(row, headers, 'answer', { answer: 1 });

  assert.equal(result.value, null);
  assert.equal(result.method, 'not_found');
});

test('extractFieldValueUnified: returns not_found when field unknown', () => {
  const ctx = loadColumnMappingContext();
  const headers = ['名前', '回答'];
  const row = ['田中', 'answer'];

  const result = ctx.extractFieldValueUnified(row, headers, 'nonexistent');

  assert.equal(result.value, null);
  assert.equal(result.method, 'not_found');
});

test('extractFieldValueUnified: preserves non-string values (numbers)', () => {
  const ctx = loadColumnMappingContext();
  const headers = ['名前', 'スコア'];
  const row = ['田中', 42];
