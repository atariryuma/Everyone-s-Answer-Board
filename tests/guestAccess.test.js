...
describe('ゲストアクセステスト', () => {
            ...
            this._access = {
              verifyAccess: (userId, action, userEmail) => {
                if (userId === 'public_user') {
                  ...
                  if (userEmail === 'owner@example.com')
                    return {
                      allowed: true,
                      userType: 'owner',
                      config: {
  ...
  describe('公開ボードのゲストアクセス', () => {
    ...
    test('所有者は自分のボードに全アクセス権限がある', () => {
      const viewResult = context.App.getAccess().verifyAccess(
        'public_user',
        'view',
        'owner@example.com'
      );
      const editResult = context.App.getAccess().verifyAccess(
        'public_user',
        'edit',
        'owner@example.com'
      );
      const adminResult = context.App.getAccess().verifyAccess(
        'public_user',
        'admin',
        'owner@example.com'
      );
  ...
  describe('プライベートボードのアクセス制御', () => {
    ...
    test('認証ユーザーもプライベートボードにアクセスできない', () => {
      const result = context.App.getAccess().verifyAccess(
        'private_user',
        'view',
        'other@example.com'
      );
